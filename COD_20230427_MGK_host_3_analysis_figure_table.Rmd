...
# Loading files -----------------------------------------------------------
#loading tidy phyloseq object
tinyvamp_host_zero <- read_rds("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/4_Data/2_Tidy/HOST_tinyvamp_decontaminated_Amy_Willis/hostzero_p_hats.RDS") %>%
        t()

tinyvamp_untreated <- read_rds("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/4_Data/2_Tidy/HOST_tinyvamp_decontaminated_Amy_Willis/untreated_p_hats.RDS") %>%
        t()

tinyvamp_qiaamp <- read_rds("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/4_Data/2_Tidy/HOST_tinyvamp_decontaminated_Amy_Willis/qiaamp_p_hats.RDS") %>%
        t()

tinyvamp_lypma <- read_rds("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/4_Data/2_Tidy/HOST_tinyvamp_decontaminated_Amy_Willis/lympa_p_hats.RDS") %>%
        t()

tinyvamp_molysis <- read_rds("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/4_Data/2_Tidy/HOST_tinyvamp_decontaminated_Amy_Willis/molysis_p_hats.RDS") %>% 
        t()
...
phyloseq_tv <- merge_phyloseq(merge_phyloseq(sample_data_tv, otu_table(tinyvamp_host_zero, taxa_are_rows = T)),
               merge_phyloseq(sample_data_tv, otu_table(tinyvamp_lypma, taxa_are_rows = T)),
               merge_phyloseq(sample_data_tv, otu_table(tinyvamp_molysis, taxa_are_rows = T)),
               merge_phyloseq(sample_data_tv, otu_table(tinyvamp_qiaamp, taxa_are_rows = T)),
               merge_phyloseq(sample_data_tv, otu_table(tinyvamp_untreated, taxa_are_rows = T)),
...
## Host ratio qPCR vs sequencing (Figure S3)
...
**Figure S3.** (A) Correlation between host DNA proportion measured with qPCR and metagenomic next generation sequencing (mNGS) and (B) Bland-Altman plot of host DNA % of qPCR and mNGS.
...
phyloseq$phyloseq_count_prev <- prune_taxa(subset(red_flag_taxa,
                                           red_flag_taxa$red_flag_decontam != 1)$species,
                                    phyloseq$phyloseq_count)
#phyloseq for analysis
phyloseq$phyloseq_count <- prune_taxa(subset(red_flag_taxa,
                                           red_flag_taxa$red_flag_prev_abd != 1 & !red_flag_taxa$red_flag_decontam)$species,
                                    phyloseq$phyloseq_count)
...
fig2 <- ggarrange(c, c %>% g_legend() %>% as_ggplot,
                  d, d %>% g_legend() %>% as_ggplot,
                  e, e %>% g_legend() %>% as_ggplot,
                  a, a %>% g_legend() %>% as_ggplot,
                  b, b %>% g_legend() %>% as_ggplot,
...
### LM on M-H distance (Table S7)

**Table S7**. Effect size, standard error (SE) and p-value of a statistical test for Horn-Morisita distance from untreated to each treated within subject (BC-distance wihtin subject by treatment~ sample type + treatment + sampetype * treatment + log10 (Final_reads) + (1|subject_id) ).
...
### List of DA taxa (Table S8)

**Table S8.** List of differentailly abundant taxa identified by MaAsLin. Significant associations (q < 0.1) that is showing high change (|log2-fold change| > 0.5) were listed.
...
### Function richness - all & stratified (Table S9)

**Table S9.** Effect size, standard error (SE) and p-value of a statistical test for function richness with an interaction term using linear mixed effect model (Species richness ~ treatment + (1|subject_id) ).
...
sr_lmer_all <- lmer(S.obs ~ treatment + (1|subject_id), data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("BAL", "Nasal", "Sputum"))) %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
...
tables9 <- cbind(sr_lmer_all, sr_lmer_bal, sr_lmer_ns, sr_lmer_spt) %>%
    kbl(format = "html", escape = 0) %>%
...
### Function beta - stratified (Table S10)


**Table S10**. Degree of freedom, effect size (residual, R^2) and p-value of permutational ANOVA for functional Horn-Morisita distances with an interaction term and strata term (BC-distance of functions ~ sample type * treatment + log10(final reads), strata = subject id).
