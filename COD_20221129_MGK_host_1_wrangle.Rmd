...
sample_names(phyloseq_path_rpkm) <- sample_data(phyloseq_path_rpkm)$baylor_other_id
...
***4. Adding tree file***
```{r}
tree <- read_tree("/Users/minsikkim/Downloads/mpa_v31_CHOCOPhlAn_201901_species_tree.nwk.txt")
tree$tip.label %>% str_split_fixed(., "[|]*__", 10) %>% .[,8] %>% duplicated %>% sum
tree$tip.label <- tree$tip.label %>% str_split_fixed(., "[|]*__", 10) %>% .[,8] %>% ifelse(!duplicated(.), ., paste(., "1", sep = ""))
tree$tip.label <- tree$tip.label %>% ifelse(!duplicated(.), ., paste(., "1", sep = ""))
tree$tip.label <- tree$tip.label %>% ifelse(!duplicated(.), ., paste(., "1", sep = ""))

phy$tree_phyloseq_count <- merge_phyloseq(phy$phyloseq_count, phy_tree(tree))
phy$tree_phyloseq_rel <- merge_phyloseq(phy$phyloseq_rel, phy_tree(tree))
```
