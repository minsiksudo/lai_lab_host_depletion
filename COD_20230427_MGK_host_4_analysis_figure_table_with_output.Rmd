...
sample_data(phyloseq_unfiltered$phyloseq_rel) %>% 
        data.frame %>% 
        select(Raw_reads, Final_reads, DNA_host_ng_uL, DNA_bac_16S_copies_uL, gram_neg_prop, S.obs) %>%
...
Peggy's comment on 20241114: 

With relative abundance there is no way to tell the microbial load to see if it is cross-well contamination or swapping of wells. The relative abundance for lyPMA_neg_3 and lyPMA_pos_5 could both pass for the mock community.
 
If you can export metaphlan-mapped reads, 16S qPCR counts, and human qPCR counts for all pos and neg controls we can discuss further on Monday. If you have time to regenerate a similar figure like the below


##### Storing data that Peggy requested


```{r}
phyloseq$phyloseq_count %>%
    subset_samples(baylor_other_id %in% c("lyPMA_neg_3", "molysis_pos_5")) %>%
    {
        data <- .
        taxa_to_keep <- taxa_sums(data) != 0  # Logical vector of taxa to keep
        prune_taxa(taxa_to_keep, data)        # Pass logical vector and phyloseq object
    } %>%
        otu_table %>%
        data.frame() %>% 
        write.csv(., "Project_SICAS2_microbiome/5_Scripts/MGK/Host_depletion_git/data/DAT_20241115_MGK_host_lyPMA_neg_3_and_molysis_pos_5.csv")
...
phyloseq$phyloseq_count %>%
    subset_samples(sample_type %in% c("Mock")) %>%
    {
        ...
        otu_table %>%
        data.frame(check.names = F) %>%
        write.csv(., "Project_SICAS2_microbiome/5_Scripts/MGK/Host_depletion_git/data/DAT_20241115_MGK_host_otu_mock.csv")


phyloseq$phyloseq_count %>%
    subset_samples(sample_type %in% c("Neg.")) %>%
    {
        data <- .
        taxa_to_keep <- taxa_sums(data) != 0  # Logical vector of taxa to keep
        prune_taxa(taxa_to_keep, data)        # Pass logical vector and phyloseq object
        ...
        otu_table %>%
        data.frame(check.names = F) %>%
        write.csv(., "Project_SICAS2_microbiome/5_Scripts/MGK/Host_depletion_git/data/DAT_20241115_MGK_host_otu_negative_controls.csv")


phyloseq$phyloseq_count %>%
    subset_samples(sample_type %in% c("Neg.", "Mock")) %>%
    sample_data %>%
        data.frame(check.names = F) %>%
        select(sample_type, Raw_reads, Metaphlan_mapped, DNA_host_nondil, DNA_bac_nondil, host_proportion) %>% 
        write.csv(., "Project_SICAS2_microbiome/5_Scripts/MGK/Host_depletion_git/data/DAT_20241115_MGK_host_neg_pos_sampledata.csv")
...
##### Raw reads of negative controls
...
phyloseq$phyloseq_count %>%
        ...
        sample_data %>%
        data.frame(check.names = F) %>%
        mutate(sample_type2 = case_when(baylor_other_id == "lyPMA_neg_3"  ~ "Ambiguous Neg.",
                                       baylor_other_id == "lyPMA_pos_1"  ~ "Ambiguous Mock",
                                       baylor_other_id == "lyPMA_pos_3"  ~ "Ambiguous Mock",
                                       baylor_other_id == "lyPMA_pos_5"  ~ "Ambiguous Mock",
                                       .default = sample_type)) %>%
        ggplot(., aes(x = baylor_other_id, y = Raw_reads, label = baylor_other_id)) +
        geom_point() +
...
##### Bacterial qPCR of negative controls
...
phyloseq$phyloseq_count %>%
        ...
        mutate(sample_type2 = case_when(baylor_other_id == "lyPMA_neg_3"  ~ "Ambiguous Neg.",
                                       ...
                                       baylor_other_id == "lyPMA_pos_5"  ~ "Ambiguous Mock",
                                       .default = sample_type)) %>%
        ggplot(., aes(x = baylor_other_id, y = DNA_bac_16S_copies_uL, label = baylor_other_id)) +
        geom_point() +
        scale_y_log10() +
        ggrepel::geom_text_repel(max.overlaps = 15,show.legend  = F) +
        ylab("Bacterial DNA measured by qPCR (copies/µL)") +
        theme_classic() +
        theme(axis.text.x = element_blank())
```

##### Raw reads of positive and negative controls
...
phyloseq$phyloseq_count %>%
        ...
        sample_data %>%
        data.frame(check.names = F) %>%
        ggplot(., aes(x = baylor_other_id, y = Raw_reads, label = baylor_other_id, col = sample_type)) +
        geom_point() +
        scale_color_manual(values = MetBrewer::met.brewer("Austria", 4)[3:4],
                           name = "Sample type",
                           breaks = c("Neg.","Mock"),
                           labels = c("Neg.","Mock")) + #color using https://colorbrewer2.org/#type=qualitative&scheme=Set1&n=6
        ggrepel::geom_text_repel(max.overlaps = 10,show.legend  = F) +
        theme_classic() +
        scale_y_log10() +
        ylab("Raw reads") +
        theme(axis.text.x = element_blank()) +
...
```
`lyPMA_neg_3` will be noted as `ambiguous negative control`

`lyPMA_pos_1`, `lyPMA_pos_3`, `lyPMA_pos_5 ` will be noted as `ambiguous negative control

##### Bacterial qPCR of positive and negative controls
...
phyloseq$phyloseq_count %>%
        ...
        mutate(sample_type2 = case_when(baylor_other_id == "lyPMA_neg_3"  ~ "Ambiguous Neg.",
                                       ...
                                       baylor_other_id == "lyPMA_pos_5"  ~ "Ambiguous Mock",
                                       .default = sample_type),
               sample_label = case_when(baylor_other_id == "lyPMA_neg_3"  ~ "lyPMA_neg_3",
                                       baylor_other_id == "lyPMA_pos_1"  ~ "lyPMA_pos_1",
                                       baylor_other_id == "lyPMA_pos_3"  ~ "lyPMA_pos_3",
                                       baylor_other_id == "lyPMA_pos_5"  ~ "lyPMA_pos_5",
                                       .default = NA)) %>%
        ggplot(., aes(x = baylor_other_id, y = DNA_bac_16S_copies_uL, label = baylor_other_id, col = sample_type2)) +
        geom_point() +
        scale_color_manual(values = MetBrewer::met.brewer("Austria", 4),
                           name = "Sample type",
                           breaks = c("Neg.","Mock", "Ambiguous Neg.", "Ambiguous Mock"))+
        labs(col="Sample type") +
        ggrepel::geom_text_repel(max.overlaps = 10,show.legend  = F) +
        ylab("Bacterial DNA measured by qPCR (copies/µL)") +
        scale_y_log10 () +
        ...
        facet_wrap(~sample_type, scale = "free_x")
```
# Is low bacterial DNA in lyPMA group is due to the treatment? 

Facetting by treatment type
...
phyloseq$phyloseq_count %>%
        ...
        sample_data %>%
        data.frame(check.names = F) %>%
        mutate(sample_type = case_when(baylor_other_id == "lyPMA_neg_3"  ~ "Ambiguous Neg.",
                                       baylor_other_id == "lyPMA_pos_1"  ~ "Ambiguous Mock",
                                       baylor_other_id == "lyPMA_pos_3"  ~ "Ambiguous Mock",
                                       baylor_other_id == "lyPMA_pos_5"  ~ "Ambiguous Mock",
                                       .default = sample_type),
               sample_label = case_when(baylor_other_id == "lyPMA_neg_3"  ~ "lyPMA_neg_3",
                                       ...
                                       baylor_other_id == "lyPMA_pos_3"  ~ "lyPMA_pos_3",
                                       .default = NA)) %>%
        ggplot(., aes(x = baylor_other_id, y = DNA_bac_16S_copies_uL, label = sample_label, col = sample_type)) +
        geom_point() +
        scale_color_manual(values = MetBrewer::met.brewer("Austria", 4),
                           name = "Sample type",
                           breaks = c("Neg.","Mock", "Ambiguous Neg.", "Ambiguous Mock"))+
        ggrepel::geom_text_repel(max.overlaps = 30, show.legend  = F, nudge_y = 5000000) + 
        theme_classic() +
        theme(axis.text.x = element_blank()) +
...
#### Beta diversity plot (Morishita-Horn)

```{r}

phyloseq$phyloseq_count %>%
        subset_samples(extraction_date == "20230119") %>% {
                temp_phyloseq <- .
                sample_data_temp <- sample_data(temp_phyloseq) %>%
                        data.frame(check.names = F)
                sample_data_temp <- sample_data_temp %>%
                        mutate(sample_type = case_when(baylor_other_id == "lyPMA_neg_3"  ~ "Ambiguous Neg.",
                                       baylor_other_id == "lyPMA_pos_1"  ~ "Ambiguous Mock",
                                       baylor_other_id == "lyPMA_pos_3"  ~ "Ambiguous Mock",
                                       baylor_other_id == "lyPMA_pos_5"  ~ "Ambiguous Mock",
                               ...
                               sample_label = case_when(
                                       baylor_other_id == "lyPMA_neg_3"  ~ "lyPMA_neg_3",
                                       baylor_other_id == "lyPMA_pos_1"  ~ "lyPMA_pos_1",
                                       baylor_other_id == "lyPMA_pos_3"  ~ "lyPMA_pos_3",
                                       baylor_other_id == "lyPMA_pos_5"  ~ "lyPMA_pos_5",
                                       baylor_other_id == "molysis_pos_5"  ~ "molysis_pos_5",
                                       .default = NA),
                               sample_label2 = case_when(
                                       treatment == "lyPMA"  ~ baylor_other_id,
                                       treatment == "MolYsis"  ~ baylor_other_id,
                                       .default = NA))
                temp_phyloseq <- merge_phyloseq(sample_data(sample_data_temp), otu_table(temp_phyloseq))
        } %>% {
                temp_phyloseq <- .
        plot <- ordinate(temp_phyloseq, method = "PCoA", distance = "horn") %>%
        plot_ordination(temp_phyloseq, ., col = "sample_type") +
        scale_color_manual(values = MetBrewer::met.brewer("Austria", 4),
                           name = "Sample type",
                           breaks = c("Neg.","Mock", "Ambiguous Neg.", "Ambiguous Mock"))+
        geom_point(size = 3) +
        theme_classic (base_size = 12, base_family = "sans") +
        ggrepel::geom_text_repel(aes(label = sample_label2), max.overlaps = 30, show.legend = F, nudge_x = -0.3) #+
        #facet_wrap(~sample_type, scales = "free") +
        #labs(tag = "A") +
        #ggtitle("PCoA plot, M-H dissimilarity, extracted on 20240119") +
...
phyloseq$phyloseq_count %>%
        subset_samples(extraction_date == "20230119") %>% {
                ...
                temp_phyloseq <- .
        plot <- ordinate(temp_phyloseq, method = "PCoA", distance = "jaccard") %>%
        plot_ordination(temp_phyloseq, ., col = "sample_type") +
        scale_color_manual(values = MetBrewer::met.brewer("Austria", 4),
                           name = "Sample type",
                           breaks = c("Neg.","Mock", "Ambiguous Neg.", "Ambiguous Mock"))+
        geom_point(size = 3) +
        theme_classic (base_size = 12, base_family = "sans") +
        ggrepel::geom_text_repel(aes(label = sample_label2), max.overlaps = 30, show.legend = F, nudge_x = -0.2) #+
        #facet_wrap(~sample_type, scales = "free") +
        #labs(tag = "A") +
        #ggtitle("PCoA plot, M-H dissimilarity, extracted on 20240119") +
        theme(plot.tag = element_text(size = 15), legend.position = "top") #+
...
```
#### Barplot of lyPMA controls and molysis_pos_5 (adjacent to lyPMA_neg_3)
...
tax_table(subset_samples(phyloseq$phyloseq_rel,
                  ...
                  subset_samples(.,
                                 (sample_type == "Mock" & treatment == "Untreated") | treatment == "lyPMA" | treatment == "MolYsis")) %>%
        cbind(species20 = "[Other]") %>%
        {top20species <- head(
                taxa_sums(
                        subset_samples(
   ...
   phyloseq_temp
        ...
        facet_wrap(sample_type~treatment, scales = "free_x", ncol = 3)
        #ggtitle("D    Mock")
        #facet_wrap (~ factor(sample_type, levels = c("Mock", "BAL", "Nasal", "Sputum")),
        #            scales= "free_x", nrow=2) +
