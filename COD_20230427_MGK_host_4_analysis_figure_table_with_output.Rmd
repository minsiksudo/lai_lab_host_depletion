...
phyloseq$phyloseq_count %>%
        subset_samples(extraction_date == "20230119") %>% {
                temp_phyloseq <- .
                sample_data(temp_phyloseq)$sample_type <- ifelse(sample_data(temp_phyloseq)$baylor_other_id == "lyPMA_neg_3", 
                                                        "Contaminated Neg.",
                                                        as.character(sample_data(temp_phyloseq)$sample_type))
                temp_phyloseq
        } %>% {
                temp_phyloseq <- .
        ...
        scale_color_manual(values = c("#a8ddb5", "#43a2ca", "#a50f15", "#fc9272", "#fee0d2"),
                           name = "Sample type",
                           breaks = c("Neg.","Mock", "Contaminated Neg."),
                           labels = c("Neg.","Mock", "Contaminated Neg.")) + #color using https://colorbrewer2.org/#type=qualitative&scheme=Set1&n=6
        #scale_shape(name = "Sample type", labels = c("Mock theoretical", "Mock")) +
        geom_point(size = 3) +
        theme_classic (base_size = 12, base_family = "sans") +
        ggrepel::geom_text_repel(aes(label = baylor_other_id), max.overlaps = 30) #+
