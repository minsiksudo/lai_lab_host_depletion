...
table1 <- sample_data %>% data.frame() %>% 
        ...
        summarise(N = n(),
              ...
              `Clean reads<br>reads x 10<sup>7</sup>` = paste(format(round(median(Reads_after_trim/10000000),2), nsmall = 2, big.mark = ","), "<br>(", format(round(quantile(Reads_after_trim/10000000, 0.25),2), nsmall = 2, big.mark = ","), ", ", format(round(quantile(Reads_after_trim/10000000, 0.75),2), nsmall = 2, big.mark = ","), ")", sep = ""),
              `Host reads ratio` = paste(format(round(median(sequencing_host_prop),2), nsmall = 2, big.mark = ","), "<br>(", format(round(quantile(sequencing_host_prop, 0.25),2), nsmall = 2, big.mark = ","), ", ", format(round(quantile(sequencing_host_prop, 0.75),2), nsmall = 2, big.mark = ","), ")", sep = ""),
...
save_kable(table1, file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/table1r.html", self_contained = T)
...
hr_lmer_bal <- lmer(sequencing_host_prop ~ treatment + (1|subject_id), 
                    data = sample_data %>%
                            data.frame %>%
                            mutate(total_read = phyloseq$phyloseq_count %>%
                            otu_table %>%
...
hr_lmer_ns <- lmer(sequencing_host_prop ~ treatment + (1|subject_id), data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("Nasal"))) %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
...
hr_lmer_spt <- lmer(sequencing_host_prop ~ treatment + (1|subject_id), data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("Sputum"))) %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>%
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
...
tables3 <- cbind(hr_lmer_bal, hr_lmer_ns, hr_lmer_spt) %>%
    kbl(format = "html", escape = 0) %>%
        add_header_above(c(" " = 1, "BAL" = 3, "Nasal swab" = 3,"Sputum"= 3)) %>% 
        kable_styling(full_width = 0, html_font = "serif")
tables3
#save_kable(tables3, file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/tableS3r.html", self_contained = T)


```


### Host ratio (stratified)
...
### Final reads (stratified)

lmer( Host DNA ratio ~ treatment + (1\|subject_id) )

**Host zero and Molysis changed host ratio**

```{r warning=F, 'LMER host % BAL1'}
tables3 <- lmer(Final_reads/10000000 ~ sample_type + treatment + sample_type * treatment + (1|subject_id),
     data = sample_data %>%
             data.frame %>%
             mutate(total_read = phyloseq$phyloseq_count %>%
                            otu_table %>%
...
fr_lmer_bal <- lmer(Final_reads/10000000 ~ treatment + (1|subject_id),
                    data = sample_data %>%
                                data.frame %>%
                                mutate(total_read = phyloseq$phyloseq_count %>%
                                otu_table %>%
...
fr_lmer_ns <- lmer(Final_reads/10000000 ~ treatment + (1|subject_id), data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("Nasal"))) %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
...
fr_lmer_spt <- lmer(Final_reads/10000000 ~ treatment + (1|subject_id), data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("Sputum"))) %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>%
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
...
table1 <- sample_data(phyloseq$phyloseq_count_prev) %>% data.frame() %>% 
        ...
        summarise(N = n(),
              ...
              `Host reads ratio` = paste(format(round(median(sequencing_host_prop),2),
                                                nsmall = 2, big.mark = ","),
                                         " (",
                                         format(round(quantile(sequencing_host_prop,
                                                               0.25),
                                                      2),
                                                nsmall = 2,
                                                big.mark = ","), 
                                         ", ", 
                                         format(round(quantile(sequencing_host_prop, 0.75),2), 
                                                nsmall = 2, 
                                                big.mark = ","), 
                                         ")", 
                                         sep = ""),
...
sample_data %>% subset(., .$S.obs != 0) %>% group_by(sample_type, treated) %>% summarise(med = median(DNA_host_nondil + DNA_bac_nondil), lo = quantile(DNA_host_nondil + DNA_bac_nondil, 0.25), high = quantile(DNA_host_nondil + DNA_bac_nondil, 0.75))

sample_data %>% subset(., .$S.obs != 0) %>%
        group_by(sample_type, treated) %>% summarise(med = median(DNA_host_nondil), lo = quantile(DNA_host_nondil, 0.25), high = quantile(DNA_host_nondil, 0.75))
...
sample_data %>% subset(., .$S.obs != 0) %>% group_by(sample_type, treated) %>% summarise(med = median(DNA_bac_nondil), lo = quantile(DNA_bac_nondil, 0.25), high = quantile(DNA_bac_nondil, 0.75))
...
### Rarefraction curve of species richenss
```{r}
...
fig_rarefraction_function <- phyloseq$phyloseq_path_rpkm %>% 
        sample_data %>% 
        data.frame %>%
        subset(., !is.na(.$treatment) & sample_type %in% c("BAL", "Nasal", "Sputum")) %>%
ggplot(., aes(x = Final_reads/1000000, y = S.obs, col = treatment)) +
...
subset(sample_data, sample_data$sample_type %in% c("BAL", "Nasal", "Sputum")) %>% .$log_centered_final_reads %>% hist (main = "Hist. of centered log10 final reads of BAL, Nasal, Sputum")

subset(sample_data, sample_data$sample_type %in% c("BAL")) %>% .$log_centered_final_reads %>% hist (main = "Hist. of centered log10 final reads of BAL")

subset(sample_data, sample_data$sample_type %in% c("Nasal")) %>% .$log_centered_final_reads %>% hist (main = "Hist. of centered log10 final reads of Nasal")

subset(sample_data, sample_data$sample_type %in% c("Sputum")) %>% .$log_centered_final_reads %>% hist (main = "Hist. of centered log10 final reads of Sputum")
...
tableS5 <- lmer(S.obs ~ sample_type * treatment + log_centered_final_reads + (1|subject_id), data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("BAL", "Nasal", "Sputum") & S.obs != 0)) %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
                               
        rownames_to_column(var = "x") %>% mutate(x = gsub("treatment|sample_type", "", x)) %>% mutate(x = gsub(":", " * ", x)) %>%
        mutate(x = gsub("log_centered_final_reads", "log<sub>10</sub>(Final reads)", x)) %>%
        column_to_rownames(var = "x") %>% 
        #mutate(across(is.numeric, round, digits=2)) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
                               abs(`Pr(>|t|)`) < 0.01 ~ "**",
...
```
### Species richness (stratified) - without sequencing depth adjusting
...
save_kable(table2, file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/table2R.html", self_contained = T)
...
filt_fit_data_ns %>% subset(., .$qval < 0.1 & .$metadata == "log10.Final_reads") %>% .$feature %>% table

filt_fit_data_spt %>% subset(., .$qval < 0.1) %>% .$feature %>% unique
filt_fit_data_spt %>% subset(., .$qval < 0.1 & .$metadata != "log10.Final_reads") %>% .$metadata %>% table
filt_fit_data_spt %>% subset(., .$qval < 0.1 & .$metadata == "log10.Final_reads") %>% .$feature %>% table
...
sample_data <- sample_data(phyloseq$phyloseq_path_rpkm)
sample_data$log_centered_final_reads <- log(sample_data$Final_reads + 1) - median(log((subset(sample_data, sample_data$sample_type %in% c("BAL") & sample_data$treatment %in% c("Untreated")) %>% .$Final_reads) + 1))


tableS8 <- lmer(S.obs ~ sample_type * treatment + log_centered_final_reads + (1|subject_id), data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("BAL", "Nasal", "Sputum"))) %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
...
### Additional calculations for the main text

```{r}
sputum_genus <- phyloseq$phyloseq_count %>% subset_samples(., sample_type == "Sputum") %>% phyloseq::tax_glom("Genus")
taxa_names(sputum_genus) <- sputum_genus %>% tax_table() %>% data.frame() %>% .$Genus
sputum_genus <- alpha_diversity(sputum_genus)

sputum_genus %>% group_by(treatment) %>% summarize(`Species richenss` = median(S.obs.y),
                                                   low = quantile(S.obs.y, 0.25),
                                                   high = quantile(S.obs.y, 0.75))
```
...
table_negs <- sample_data(phyloseq$phyloseq_count) %>% data.frame() %>% 
        dplyr::filter(sample_type %in% c("Neg.", "Sputum", "Nasal", "BAL"), !is.na(original_sample)) %>% 
        group_by (sample_type, treatment) %>%
        summarise(N = n(),
                  `Species richness` = paste(format(round(median(S.obs),0), nsmall = 0, big.mark = ","),
                                             " (", format(round(quantile(S.obs, 0.25),0), nsmall = 0, big.mark = ","), ", ",
                                             format(round(quantile(S.obs, 0.75),0), nsmall = 0, big.mark = ","), 
                                             ")", sep = ""),
                  `Final reads x 10<sup>6</sup>` = paste(format(round(median(Final_reads/1000000),2), nsmall = 2, big.mark = ","), " (", format(round(quantile(Final_reads/1000000, 0.25),2), nsmall = 2, big.mark = ","), ", ", format(round(quantile(Final_reads/1000000, 0.75),2), nsmall = 2, big.mark = ","), ")", sep = "")
                  ) %>%
        select(-c("sample_type")) %>%
        kbl(format = "html", escape = 0, caption = "Negs & samples") %>%
        kable_styling(full_width = 0, html_font = "serif")
table_negs %>%
        save_kable(., file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/negs_and_samples.html", self_contained = T)
table_negs

phyloseq_negs <- subset_samples(phyloseq_unfiltered$phyloseq_count, sample_type %in% c("Neg.") & !is.na(original_sample))

phyloseq_negs_untreated <- subset_samples(phyloseq_negs, treatment == "Untreated")
phyloseq_negs_untreated <- prune_taxa(taxa_sums(phyloseq_negs_untreated) > 0, phyloseq_negs_untreated)
phyloseq_negs_lypma <- subset_samples(phyloseq_negs, treatment == "lyPMA")
phyloseq_negs_lypma <- prune_taxa(taxa_sums(phyloseq_negs_lypma) > 0, phyloseq_negs_lypma)
phyloseq_negs_benzonase <- subset_samples(phyloseq_negs, treatment == "Benzonase")
phyloseq_negs_benzonase <- prune_taxa(taxa_sums(phyloseq_negs_benzonase) > 0, phyloseq_negs_benzonase)
phyloseq_negs_hostzero <- subset_samples(phyloseq_negs, treatment == "Host zero")
phyloseq_negs_hostzero <- prune_taxa(taxa_sums(phyloseq_negs_hostzero) > 0, phyloseq_negs_hostzero)
phyloseq_negs_molysis <- subset_samples(phyloseq_negs, treatment == "Molysis")
phyloseq_negs_molysis <- prune_taxa(taxa_sums(phyloseq_negs_molysis) > 0, phyloseq_negs_molysis)
phyloseq_negs_qiaamp <- subset_samples(phyloseq_negs, treatment == "QIAamp")
phyloseq_negs_qiaamp <- prune_taxa(taxa_sums(phyloseq_negs_qiaamp) > 0, phyloseq_negs_qiaamp)



#Untreated
phyloseq_negs_untreated <- cbind(transform_sample_counts(phyloseq_negs_untreated, function(x) {x/sum(x)}) %>% 
              otu_table %>% 
              t %>% 
              data.frame() %>% 
              mutate(`Total sum` = rowSums(.), .before = Micrococcus_luteus) %>% t,
      phyloseq_negs_untreated %>% 
              otu_table %>% 
              t %>% 
              data.frame() %>% 
              mutate(`Total sum` = rowSums(.), .before = Micrococcus_luteus) %>% 
              t) %>% data.frame() %>%
        mutate(across(is.numeric, round, digits=3)) %>%
        kbl(format = "html", escape = 0, caption = "Untreated Negs") %>%
        add_header_above(c("Taxa", "Relative abundance" = 5,"Read count" = 5)) %>% 
        kable_styling(full_width = 0, html_font = "serif")

phyloseq_negs_untreated %>% 
        save_kable(., file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/neg_table_untreated.html", self_contained = T)
phyloseq_negs_untreated
        
#lyPMA


phyloseq_negs_lypma <- cbind(transform_sample_counts(phyloseq_negs_lypma, function(x) {x/sum(x)}) %>% 
              otu_table %>% 
              t %>% 
              data.frame() %>% 
              mutate(`Total sum` = rowSums(.), .before = Rothia_mucilaginosa) %>% t,
      phyloseq_negs_lypma %>% 
              otu_table %>% 
              t %>% 
              data.frame() %>% 
              mutate(`Total sum` = rowSums(.), .before = Rothia_mucilaginosa) %>% 
              t) %>% data.frame() %>%
        mutate(across(is.numeric, round, digits=3)) %>%
        kbl(format = "html", escape = 0, caption = "lyPMA Negs") %>%
        add_header_above(c("Taxa", "Relative abundance" = 5,"Read count" = 5)) %>% 
        kable_styling(full_width = 0, html_font = "serif")

phyloseq_negs_lypma%>%
        save_kable(., file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/neg_table_lypma.html", self_contained = T)

phyloseq_negs_lypma

#BAL
phyloseq_negs_benzonase <- cbind(transform_sample_counts(phyloseq_negs_benzonase, function(x) {x/sum(x)}) %>% 
              otu_table %>% 
              t %>% 
              data.frame() %>% 
              mutate(`Total sum` = rowSums(.), .before = Cutibacterium_acnes) %>% t,
      phyloseq_negs_benzonase %>% 
              otu_table %>% 
              t %>% 
              data.frame() %>% 
              mutate(`Total sum` = rowSums(.), .before = Cutibacterium_acnes) %>% 
              t) %>% data.frame() %>%
        mutate(across(is.numeric, round, digits=3)) %>%
        kbl(format = "html", escape = 0, caption = "Benzonase Negs") %>%
        add_header_above(c("Taxa", "Relative abundance" = 5,"Read count" = 5)) %>% 
        kable_styling(full_width = 0, html_font = "serif")

phyloseq_negs_benzonase %>%
        save_kable(., file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/neg_table_benzonase.html", self_contained = T)

phyloseq_negs_benzonase
#Host zero
phyloseq_negs_hostzero <- cbind(transform_sample_counts(phyloseq_negs_hostzero, function(x) {x/sum(x)}) %>% 
              otu_table %>% 
              t %>% 
              data.frame() %>% 
              mutate(`Total sum` = rowSums(.), .before = Cutibacterium_acnes) %>% t,
      phyloseq_negs_hostzero %>% 
              otu_table %>% 
              t %>% 
              data.frame() %>% 
              mutate(`Total sum` = rowSums(.), .before = Cutibacterium_acnes) %>% 
              t) %>% data.frame() %>%
        mutate(across(is.numeric, round, digits=3)) %>%
        kbl(format = "html", escape = 0, caption = "Host zero Negs") %>%
        add_header_above(c("Taxa", "Relative abundance" = 5,"Read count" = 5)) %>% 
        kable_styling(full_width = 0, html_font = "serif")
phyloseq_negs_hostzero %>%
        save_kable(., file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/neg_table_hostzero.html", self_contained = T)
phyloseq_negs_hostzero
        
 #Molysis        
phyloseq_negs_molysis <- cbind(transform_sample_counts(phyloseq_negs_molysis, function(x) {x/sum(x)}) %>% 
              otu_table %>% 
              t %>% 
              data.frame() %>% 
              mutate(`Total sum` = rowSums(.), .before = Cutibacterium_acnes) %>% t,
      phyloseq_negs_molysis %>% 
              otu_table %>% 
              t %>% 
              data.frame() %>% 
              mutate(`Total sum` = rowSums(.), .before = Cutibacterium_acnes) %>% 
              t) %>% data.frame() %>%
        mutate(across(is.numeric, round, digits=3)) %>%
        kbl(format = "html", escape = 0, caption = "Molysis Negs") %>%
        add_header_above(c("Taxa", "Relative abundance" = 5,"Read count" = 5)) %>% 
        kable_styling(full_width = 0, html_font = "serif")
phyloseq_negs_molysis %>%
        save_kable(., file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/neg_table_molysis.html", self_contained = T)
phyloseq_negs_molysis
#QIAamp
phyloseq_negs_qiaamp <- cbind(transform_sample_counts(phyloseq_negs_qiaamp, function(x) {x/sum(x)}) %>% 
              otu_table %>% 
              t %>% 
              data.frame() %>% 
              mutate(`Total sum` = rowSums(.), .before = Micrococcus_luteus) %>% t,
      phyloseq_negs_qiaamp %>% 
              otu_table %>% 
              t %>% 
              data.frame() %>% 
              mutate(`Total sum` = rowSums(.), .before = Micrococcus_luteus) %>% 
              t) %>% data.frame() %>%
        mutate(across(is.numeric, round, digits=3)) %>%
        kbl(format = "html", escape = 0, caption = "QIAamp Negs") %>%
        add_header_above(c("Taxa", "Relative abundance" = 5,"Read count" = 5)) %>% 
        kable_styling(full_width = 0, html_font = "serif")
phyloseq_negs_qiaamp %>%
        save_kable(., file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/neg_table_qiaamp.html", self_contained = T)
phyloseq_negs_qiaamp
