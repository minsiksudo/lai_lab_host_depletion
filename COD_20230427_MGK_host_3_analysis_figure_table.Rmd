...
**Table S3.** Changes on final reads stratified by sample type tested with linear mixed effect models (log10(Final reads) ~ Treatment + (1|Subject id)). Effect size with adjusted 95% confidence intervals and p-value were listed. The unit of final read is reads x 106. Statistical significances were noted with `*`: p-value < 0.05, `**`: p-value < 0.01 and `***`: p-value < 0.001.
...
### Alpha and beta diversity (Figure)
...
## LME for speceis richness {.tabset}                       

### distribution of final reads
...
lmer(S.obs ~ sample_type * treatment + log_centered_final_reads + (1|subject_id), data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("BAL", "Nasal", "Sputum") & S.obs != 0)) %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
...
Degree of freedom, effect size (residual, R^2) and p-value of permutational ANOVA for Bray-curtis distiances with an interaction term and strata term (BC-distance ~ sample_type * treatment + log10 (Final_reads), strata = subject_id).
...
## Function alaph and beta diversity (figure S8) {.tabset}
...
# *vii.	What type of fuction were affected by the treatment?*

*vii.	What type of fucntion were affected by the treatment?*
