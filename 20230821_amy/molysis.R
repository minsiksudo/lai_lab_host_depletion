...
ps_molysis_genus <- ps$phyloseq_count %>% 
  tax_glom("Genus") %>%
  phyloseq::subset_samples(treatment == "Molysis")  %>%
  phyloseq::prune_taxa(taxa_sums(.) > 0, .)
zero_counts <- which(ps_molysis_genus %>% sample_sums() == 0) %>% names
ps_molysis_genus <- ps_molysis_genus %>%
  phyloseq::subset_samples(!(baylor_other_id %in% zero_counts)) %>% # has zero reads
  phyloseq::prune_taxa(taxa_sums(.) > 0, .) 
W <- ps_molysis_genus %>% otu_table %>% as.matrix %>% t
jj <- ncol(W) 
nn <- nrow(W) 
sample_ids <- ps_molysis_genus %>% sample_data %>% as_tibble %>% pull(sample_id)
subject_ids <- ps_molysis_genus %>% sample_data %>% as_tibble %>% pull(subject_id)
treatment_ids <- ps_molysis_genus %>% sample_data %>% as_tibble %>% pull(treatment)
...
colnames(P) <- ps_molysis_genus %>% otu_table %>% rownames
...
DNA_bac_ng_uLs <- ps_molysis_genus %>% sample_data %>% as_tibble %>% pull(DNA_bac_ng_uL)
...
######## Step 2
molysis_all <- estimate_parameters(W = W,
                                   X = X,
                                   Z = Z,
                                   Z_tilde = Z_tilde,
...
system("say done")
molysis_all$optimization_status
...
molysis_phats <- molysis_all$P
colnames(molysis_phats) <- colnames(molysis_all$W)
rownames(molysis_phats) <- colnames(molysis_all$Z)
molysis_phats <- molysis_phats[!grepl("Neg.", rownames(molysis_phats)) & 
                                 !grepl("Mock", rownames(molysis_phats)), ]
molysis_phats %>% apply(1, sum)
(molysis_phats > 0) %>% apply(1, sum) %>% summary

saveRDS(molysis_phats, "../output/molysis_p_hats_all_v3.RDS")
