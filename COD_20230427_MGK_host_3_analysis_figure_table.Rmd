...
bray_dist_long <- distance(phyloseq$phyloseq_rel %>% subset_samples(., S.obs != 0), method="bray") %>% as.matrix() %>% melt_dist() #making long data of distance matrices
...
path_bray_dist_long_within_sampleid_from_control %>% 
  ...
  mutate(se = sd / sqrt(n),
         ...
         text = paste(sprintf("%.2f", round(mean, digits = 2)), " [", sprintf("%.2f", round(lower.ci, digits = 2)), ", ", sprintf("%.2f", round(upper.ci, digits = 2)), "]")) %>%
...
phyloseq$phyloseq_path_rpkm %>% subset_samples(., S.obs != 0 & sample_type %in% c("Sputum", "Nasal", "BAL", "Mock")) %>% sample_data %>% subset(., .$sample_type == "Nasal")
