...
cfu_per_ml_log10 ~ treatment + Frozen + `DNA/RNA shield` + Glycerol + (1|species2)
...
### Stratified by species
  ...
  model : cfu_per_ml/1000000 ~ treatment + Frozen + `DNA/RNA shield` + Glycerol

at regular scale (but devided by 10^6) as nubmers are too large
...
multi_model_sa <- hdv_all %>% 
        ...
        lm(data = .,
                       cfu_per_ml/1000000 ~ treatment + Frozen + `DNA/RNA shield` + Glycerol) 
...
lm_viability_result <- cbind(multi_sa, multi_pa, multi_a) %>%
    kbl(format = "html", escape = 0) %>%
        add_header_above(c(" " = 1, "<i>Staphylococcus aureus</i>" = 3, "<i>Pseudomonas aeruginosa</i>" = 3,"<i>Achromobacter</i> spp."= 3), escape = F) %>% 
        kable_styling(full_width = 0, html_font = "sans")

save_kable(lm_viability_result, file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/tableS12.html", self_contained = T)
...
remove DNA RNA shield from the model
...
multi_model_sa <- hdv_all %>% 
        ...
        lm(data = .,
                       cfu_per_ml_log10 ~ treatment + Frozen + `DNA/RNA shield` + Glycerol) 
...
LMER : --> Use LMER and keep `DNA/RNA shield` term.
...
multi_model_sa <- hdv_all %>% 
        ...
        lmerTest::lmer(data = .,
                       cfu_per_ml_log10 ~ treatment + Frozen + `DNA/RNA shield` + Glycerol + (1|cfb_strain)) 
...
# detailed model (stratified by all treatment group)
...
hdv_all %>%
        ...
        group_by(species2, treatment2) %>%
        summarise(mean = mean(cfu_per_ml),
                  n = n())
...
lmer_viability <- 
        ...
        mutate(treatment3 = treatment2 %>% as.numeric %>% as.factor) %>%
        lmerTest::lmer(data = .,
                       cfu_per_ml/1000000 ~ treatment3 + (1|species2)) %>%
        summary %>%
        .$coefficient %>%
        data.frame(check.names = F) 

rownames(lmer_viability) <- c("(Intercept)", "Unfrozen DNA/RNA Shield", "Frozen", "Frozen Glycerol", 
                                          "Frozen lyPMA", "Frozen Glycerol lyPMA",
                                          "Frozen Benzonase", "Frozen Glycerol Benzonase",
                                          "Frozen HostZERO", "Frozen glycerol HostZERO",
                                          #"Frozen MolYsis", "Frozen Glycerol MolYsis",
                                          "Frozen QIAamp", "Frozen Glycerol QIAamp")
lmer_viability_model <- 
        hdv_all %>%
        filter(
                species2 %in% c("Staphylococcus aureus", "Pseudomonas aeruginosa", "Achromobacter spp") &
                        !(treatment2 %in% c("Frozen MolYsis", "Frozen Glycerol MolYsis"))
...
lmer_viability_table <- lmer_viability %>%
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
                               abs(`Pr(>|t|)`) < 0.01 ~ "**",
                               abs(`Pr(>|t|)`) < 0.05 ~ "*",
                               .default = " ")) %>% 
        ...
        merge(., 
              lmer_viability_model %>% confint() %>%
                        ...
                        mutate(conf = paste("(",
                                            ...
                                            ")",
                                            sep = ""),
                               rownames_2 = c("Frozen MolYsis", "Frozen Glycerol MolYsis",
                                              "(Intercept)", "Unfrozen DNA/RNA Shield", "Frozen", "Frozen Glycerol", 
                                          "Frozen lyPMA", "Frozen Glycerol lyPMA",
                                          "Frozen Benzonase", "Frozen Glycerol Benzonase",
                                          "Frozen HostZERO", "Frozen glycerol HostZERO",
                                          "Frozen QIAamp", "Frozen Glycerol QIAamp")) %>%
                      remove_rownames() %>%
                      column_to_rownames("rownames_2"),
              by = 0
              ) %>%
        column_to_rownames("Row.names") %>%
        #rownames_to_column(var = "x") %>% mutate(x = gsub("treatment|sample_type", "", x)) %>% mutate(x = gsub(":", " * ", x)) %>%
        ...
        mutate("Effect size (95% CI)" = 
               ...
               "<i>p</i>-value" = round(`<i>p</i>-value`, 3)) %>%
        dplyr::select(c("Effect size (95% CI)", "<i>p</i>-value", " ")) %>%
        .[c("(Intercept)", "Unfrozen DNA/RNA Shield", "Frozen", "Frozen Glycerol", 
                                          "Frozen lyPMA", "Frozen Glycerol lyPMA",
                                          "Frozen Benzonase", "Frozen Glycerol Benzonase",
                                          "Frozen HostZERO", "Frozen glycerol HostZERO",
                                          #"Frozen MolYsis", "Frozen Glycerol MolYsis",
...
lmer_viability_table



hdv_all %>%
    mutate(treatment3 = factor(treatment2, 
                               levels = c("Unfrozen", "Unfrozen DNA/RNA Shield", "Frozen", "Frozen Glycerol", 
                                          "Frozen lyPMA", "Frozen Glycerol lyPMA",
                                          "Frozen Benzonase", "Frozen Glycerol Benzonase",
                                          "Frozen HostZERO", "Frozen glycerol HostZERO",
    ...
    lmerTest::lmer(data = .,
                   cfu_per_ml ~ treatment3 + (1|species2)) %>%
    summary()

lm_viability

        hdv_all$treatment2 %>% unique %>% list %>% .[[1]] %>% as.character()
        
rownames_to_column("rownames_temp") %>%
        mutate(rownames_temp = gsub("as.character\\(treatment2\\)", "", .$rownames_temp)) 
        
# Convert treatment3 to an unordered factor with specified levels
hdv_all %>%
    mutate(treatment3 = factor(treatment2, 
                               levels = c("Unfrozen", "Unfrozen DNA/RNA Shield", "Frozen", "Frozen Glycerol", 
                                          "Frozen lyPMA", "Frozen Glycerol lyPMA",
...
hdv_all %>%
        ...
        lmerTest::lmer(data = .,
                       cfu_per_ml ~ as.character(treatment2) * as.character(species2) + (1|species2)) %>%
        summary %>%
        .$coefficient
...
annotate_figure(ggarrange(rare_figure_microbe +
        ...
        common.legend = T, ncol = 1),
                left = text_grob("Species richness",
                                 ...
                                 family = "sans", 
                                 size = 11),
        bottom = text_grob("MetaPhlAn mapped reads", rot = 0, family = "sans", size = 11)
                )
