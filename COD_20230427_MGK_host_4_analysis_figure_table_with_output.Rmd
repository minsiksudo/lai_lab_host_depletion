...
figS7 <- ggplot(filt_maaslin_all, aes(y = -log10(qval), x = Estimate, col = metadata)) +
        ...
        xlab("Change estimate of CLR transformed count") +
...
tableS7 <- filt_maaslin_all %>% subset(., .$qval < 0.1 & abs(.$Estimate) > 2) %>% subset(., .$metadata != "sample_type" & .$metadata != "log10.Final_reads") %>% dplyr::select(c("feature", "metadata", "Estimate", "qval")) %>%
