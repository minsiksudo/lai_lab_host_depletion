...
red_flag_function <- data.frame(function. = function_qc$function., 
                                red_flag_prev_abd = ifelse(function_qc$prevalence < otu_table(phyloseq$phyloseq_path_rpkm) %>% 
                                t %>% 
                                rownames() %>% 
                                length * 0.05, 1, 0))# & function_qc$mean_rpkm < quantile(function_qc$mean_rpkm, 0.75), 1, 0))
...
#meta.sub <- subset(meta.sorted, meta.sorted$S.obs != 0)
dim(otu.full) #3 samples were thrown out
min(rowSums(otu.full))
meta.full <- subset(meta.sorted, meta.sorted$baylor_other_id %in% rownames(otu.full))

#Making data of t-tests - Not usfull for my analysis
#p.t.full = t.test(rowSums(otu.full) ~ meta.full$treatment, var.equal=FALSE)$p.value 
#p.wilcox.full = wilcox.test(rowSums(otu.full) ~ meta.full$treatment)$p.value
...
if (data_full==1) {
    formula = otu.tab | (subject_id + sample_type + log10(Final_reads)) ~ treatment
} else {
    formula = otu.tab | sample_type + treatment + log10(Final_reads) ~ disease
}

res.F <- permanovaFL(formula = formula,
                                data=meta.data, dist.method = dist_method,
                                #tree=crohn.tree, 
...
if (data_full==1) {
    formula = dist_sq_r | (subject_id + sample_type + log10(Final_reads)) ~ treatment
} else {
