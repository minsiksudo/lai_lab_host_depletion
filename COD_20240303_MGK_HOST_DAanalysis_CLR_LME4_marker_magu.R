...
write.csv(lmer_taxa_all, "Git/Host_depletion_macmini/data/mrkrmg_host_da_lmer_filt_maaslin_all.csv")
write.csv(lmer_taxa_bal, "Git/Host_depletion_macmini/data/mrkrmg_host_da_lmer_filt_fit_data_bal.csv")
write.csv(lmer_taxa_spt, "Git/Host_depletion_macmini/data/mrkrmg_host_da_lmer_filt_fit_data_spt.csv")
write.csv(lmer_taxa_ns, "Git/Host_depletion_macmini/data/mrkrmg_host_da_lmer_filt_fit_data_ns.csv")


######################################################################

# Analaysis on viral species level
...
dummy <- compositions::clr(otu_table(phyloseq$viral_rel)) %>% data.frame()
#dummy2 <- otu_table(physeq0_clr_all) %>% data.frame()


# Using phyloseq$viral_host_rel, do CLR transform 

physeq0_clr_all <- phyloseq$viral_rel %>%
        prune_taxa(taxa_sums(.) > 0, .) %>%
        subset_samples(., sample_type %in% c("BAL", "Nasal", "Sputum")) %>% 
        microbiome::transform(., 'clr')
physeq0_clr_all %>% otu_table
...
physeq0_clr_spt <- subset_samples(phyloseq$viral_rel, sample_type == "Sputum") %>% 
        prune_taxa(taxa_sums(.) > 0, .) %>%
        microbiome::transform(., 'clr')

physeq0_clr_bal <- subset_samples(phyloseq$viral_rel, sample_type == "BAL") %>%
        prune_taxa(taxa_sums(.) > 0, .) %>%
        microbiome::transform(., 'clr')

physeq0_clr_ns <- subset_samples(phyloseq$viral_rel, sample_type == "Nasal") %>%
        prune_taxa(taxa_sums(.) > 0, .) %>%
        microbiome::transform(., 'clr')
