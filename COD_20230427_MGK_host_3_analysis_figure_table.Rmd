...
water <- phyloseq$phyloseq_count %>%
  subset_samples(sample_type %in% c("Mock",
                                   "BAL",
                                   "Nasal",
                                   "Sputum"))
...
water_order <- GlobalPatterns %>% phyloseq::tax_glom("Order")
...
**Table S9**. Volcano plot of functional differential abundanace analysis
...
table4 <- matrix(nrow=3,ncol=5) %>% data.frame() %>% rename(lyPMA = X1, Benzonase = X2, `Host zero` = X3, Molysis = X4, QIAamp = X5) %>%
