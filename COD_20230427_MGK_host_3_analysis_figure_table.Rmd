...
lmer_a <- lmer(Final_reads/1000000 ~ treatment + (1|subject_id),
     data = sample_data %>%
             data.frame %>%
             dplyr::filter(sample_type %in% c("BAL") & S.obs != 0)) %>%
        summary %>% .$coefficients %>% data.frame(check.names = F) %>%
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.05 ~ "*", .default = " ")) %>%
        rownames_to_column(var = "x") %>% mutate(x = gsub("sample_type|treatment", "", x))  %>% mutate(x = gsub(":", " * ", x)) %>% 
        column_to_rownames(var = "x") %>% 
...
lmer_b <- lmer(Final_reads/1000000 ~ treatment + (1|subject_id),
     data = sample_data %>%
             data.frame %>%
             dplyr::filter(sample_type %in% c("Nasal"))) %>%
        summary %>% .$coefficients %>% data.frame(check.names = F) %>%
...
lmer_c <- lmer(Final_reads/1000000 ~ treatment + (1|subject_id),
     data = sample_data %>%
             data.frame %>%
             dplyr::filter(sample_type %in% c("Sputum"))) %>%
        summary %>% .$coefficients %>% data.frame(check.names = F) %>%
...
table1 <- sample_data(phyloseq$phyloseq_count_prev) %>% data.frame() %>% 
        ...
        summarise(N = n(),
              `Total DNA <br>ng/µL` = paste(format(round(median(picogreen_ng_ul),2), nsmall = 2, big.mark = ","), "<br>(", format(round(quantile(picogreen_ng_ul, 0.25),2), nsmall = 2, big.mark = ","), ", ", format(round(quantile(picogreen_ng_ul, 0.75),2), nsmall = 2, big.mark = ","), ")", sep = ""),
              `Library/<br>sequencing<br>failure<br>N (%)` = paste(sum(lib_failed + sequencing_fail), "<br>(", sum(lib_failed +sequencing_fail) / n() * 100, " %)", sep = ""),
              `Clean reads<br>reads x 10<sup>7</sup>` = paste(format(round(median(Reads_after_trim/10000000),1), nsmall = 1, big.mark = ","), "<br>(", format(round(quantile(Reads_after_trim/10000000, 0.25),1), nsmall = 1, big.mark = ","), ", ", format(round(quantile(Reads_after_trim/10000000, 0.75),1), nsmall = 1, big.mark = ","), ")", sep = ""),
              `Host reads ratio` = paste(format(round(median(sequencing_host_prop),2),
                                         ...
                                         ")", 
                                         sep = ""),
             `Final reads<br>reads x 10<sup>7</sup>` = paste(format(round(median(Final_reads/10000000, na.rm = T),2),
                                                                    nsmall = 2, big.mark = ","),
                                                             "<br>(", 
                                                             format(round(quantile(Final_reads/10000000,
                                                                                            0.25,
                                                                                            na.rm = T),
                                                                                   2),
                                                                             nsmall = 2,
...
tableS5 <- lmer(S.obs ~ sample_type * treatment + log_centered_final_reads + (1|subject_id), data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("BAL", "Nasal", "Sputum"))) %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
...
sr_lmer_bal <- lmer(S.obs ~ treatment + bal_log_centered_final_reads + (1|subject_id), data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("BAL"))) %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
