...
```{r warning=F}
lmer(sequencing_host_prop * 100 ~ treatment + (1|subject_id), 
                    data = sample_data %>%
                            data.frame %>%
                            mutate(total_read = phyloseq_unfiltered$phyloseq_count %>%
                            otu_table %>%
...
lmer(sequencing_host_prop * 100 ~ treatment + (1|subject_id), data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("Nasal"))) %>% 
        summary()
```
...
```{r}
lmer(sequencing_host_prop * 100 ~ treatment + (1|subject_id), data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("Sputum"))) %>% 
        summary() 
...
lmer(log10(Final_reads/1000000) ~ treatment + (1|subject_id),
                    data = sample_data %>%
                                data.frame %>%
                                mutate(total_read = phyloseq_unfiltered$phyloseq_count %>%
                                otu_table %>%
...
```{r}
barplot_mock_microbe <- ggarrange(
                  a, a %>% lemon::g_legend() %>% as_ggplot,
                  #b, b %>% lemon::g_legend() %>% as_ggplot,
...
**Table S5.** Degree of freedom, effect size (residual, R2) and p-value of permutational ANOVA for Morisita-Horn index with an interaction term and strata term (MH-index of species composition ~ sample type `*` treatment + subject + log10(final reads), strata = subject id). Statistical significances were noted with `***:` p-value < 0.001.
...
Degree of freedom, effect size (residual, R^2) and p-value of permutational ANOVA for Morisita-Horn distiances  for species richness stratified by sample type (MH-distance ~ lyPMA + Benzoase + HostZERO + MolYsis + QIAamp, strata = subject_id).
...
**Table S6.** Effect size, standard error (SE) and p-value of a statistical test for Morisita-Horn index from untreated to each treated within subject, stratified by sample type. Stratified analyses were conducted for each sample type as an interaction term of sample type and treatment was significant (p-value < 0.001) using a model, ANOVA(species richness ~ sample type + treatment + sample type * treatment). The tested was conducted with a linear model LM(distance within subject by treatment ~ treatment). The baseline of sample type is untreated (0 distance, untreated from untreated), and statistical significances were noted with `*: p-value < 0.05, **: p-value < 0.01 and ***: `p-value < 0.001.
...
Raw lm result on M-H distances

```{r}
lm(dist ~ treatment * sample_type, data = path_horn_dist_long_within_sampleid_from_control) %>% summary() 
...
```{r}
lm(dist ~ treatment, data = path_horn_dist_long_within_sampleid_from_control %>% subset(., .$sample_type == "BAL")) %>% summary
```
...
```{r}
lm(dist ~ treatment, data = path_horn_dist_long_within_sampleid_from_control %>% subset(., .$sample_type == "Nasal")) %>% summary
```
...
```{r}
lm(dist ~ treatment, data = path_horn_dist_long_within_sampleid_from_control %>% subset(., .$sample_type == "Sputum")) %>% summary
```
...
a <- lm(dist ~ treatment, data = path_horn_dist_long_within_sampleid_from_control %>% subset(., .$sample_type == "BAL")) %>% summary %>% .$ coefficients %>% data.frame(check.names = F) %>% rownames_to_column('row.names') %>% 
        mutate(row.names = gsub("host_zero", "HostZERO", row.names)) %>% 
...
figS7 <- ggplot(filt_maaslin_all, aes(y = -log10(qval), x = coef, col = metadata)) +
        ...
        scale_color_manual(values = c("#4daf4a",  "grey", "#f781bf", "#377eb8", "#ff7f00", "#ffff33", "#a65628"),
                           ...
                           labels = c("log<sub>10</sub>(Final reads)", "Sample type", "lyPMA", "Benzonase", "HostZERO",  "MolYsis", "QIAamp")) + #color using https://colorbrewer2.org/#type=qualitative&scheme=Set1&n=6
...
```{r}
sr_lmer_bal <- lm(S.obs ~ treatment, data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("BAL"))) 
sr_lmer_bal %>% summary
```
...
```{r}
sr_lmer_ns <- lm(S.obs ~ treatment, data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("Nasal"))) 
sr_lmer_ns %>% summary
```
...
```{r}
sr_lmer_spt <- lm(S.obs ~ treatment, data = sample_data %>% data.frame %>% subset(., .$sample_type %in% c("Sputum"))) 
sr_lmer_spt %>% summary
```
...
horn_perm_inter <- vegan::adonis2(distance(phyloseq_rel_nz, method="horn") ~ sample_type * treatment + subject_id + log10(Final_reads),
                                  data = phyloseq_rel_nz %>% sample_data %>% data.frame(check.names = F), 
...
```{r}
horn_perm_inter %>% data.frame(check.names = F) %>% rownames_to_column("row.names") %>% 
        mutate(row.names = case_when(row.names == "sample_type" ~ 'Sample type',
                                     row.names == "treatment" ~ 'Treatment',
                                     row.names == "subject_id" ~ 'Subject',
                                     row.names == "log10(Final_reads)" ~ 'log10(Final reads)',
...
figS9 <- ggplot(f_maaslin_all, aes(y = -log10(qval), x = coef, col = metadata)) +
        ...
        scale_color_manual(values = c("grey", "#f781bf", "#377eb8", "#ff7f00", "#ffff33", "#a65628"),
                           ...
                           labels = c("Sample type", "lyPMA", "Benzonase", "HostZERO",  "MolYsis", "QIAamp")) + #color using https://colorbrewer2.org/#type=qualitative&scheme=Set1&n=6
...
sr_lmer_bal_decontam <- sr_lmer_bal_decontam %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
...
sr_lmer_ns_decontam <- sr_lmer_ns_decontam %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
...
sr_lmer_spt_decontam <- sr_lmer_spt_decontam %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>%
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
...
tableA1 <- cbind(
        ...
        cbind(sr_lmer_bal_tv, sr_lmer_ns_tv, sr_lmer_spt_tv) %>% remove_rownames()) %>%
    kbl(format = "html", escape = 0) %>%
...
**MolYsis for BAL, QIAamp for nasal sab, and HostZERO for sputum. **
...
summary_bal <- matrix(nrow=5,ncol=4) %>% data.frame() %>% 
        ...
        rownames_to_column("x") %>% mutate(x = c("lyPMA", "Benzonase", "HostZERO", "MolYsis", "QIAamp"),
                                           Issues = c("High host%",
                                                     "High host%",
                                                     "-",
                                                     "-",
