...
row.names(magu_virus1) <- magu_virus1$SGB
row.names(magu_virus2) <- magu_virus2$SGB


data_virus1 <- magu_virus1 %>% select("lineage", "contig_id", "marker.magu_genes", "contig_length", "checkv_completeness", "iphop_host_genus", "iphop_confidence", "Virulence_score")

data_virus2 <- magu_virus2 %>% select("lineage", "contig_id", "marker.magu_genes", "contig_length", "checkv_completeness", "iphop_host_genus", "iphop_confidence", "Virulence_score")
...
```{r}
v_phy1 <- merge_phyloseq(v_otu1, sample_data(phyloseq_unfiltered$phyloseq_count))

dummy_phyloseq <- phyloseq_unfiltered$phyloseq_count
sample_names(dummy_phyloseq) <- sample_data(dummy_phyloseq)$baylor_id

v_phy2 <- merge_phyloseq(v_otu2, sample_data(dummy_phyloseq))
...
```{r}
saveRDS(v_phyloseq, "Project_SICAS2_microbiome/4_Data/2_Tidy/Phyloseq/PHY_20240212_MGK_host_marker_magu.rds")
```
