...
fs_viral_sr <- ggplot(sample_data %>% subset(., .$sample_type %in% c("BAL", "Nasal", "Sputum")), aes(x = sample_type, y = log10(Viral_reads))) +
...
#tiff(file = "/Users/minsikkim/Dropbox (Personal)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/FigureS3.tiff",   # The directory you want to save the file in
...
#        scale_x_discrete(drop=) +
        facet_wrap(~ treatment,  nrow = 1, drop = T, scales = "free_x") +
        ggtitle("Nasal swabs (viral host genus)") +
        labs(tag = "D")
...
e_vh <- tax_table(subset_samples(v_h_phyloseq_rel_renamed,
   ...
   phyloseq_temp
        ...
        facet_wrap(~ treatment, scales= "free_x", nrow = 1) +
        ggtitle("Sputum (viral host genus)") +
        labs(tag = "F")
...
dat_text <- data.frame(
  ...
  dist = c(
          #50, 30, 35, 33, 31,
          0.9, 0, 0, 0, 0,
          1.02, 0.4, 0, 0.73, 0.97,
          0.8, 0.65, 1, 1.04, 0.99)
)
...
filt_maaslin_all_v <- read.csv("Git/Host_depletion_macmini/data/mrkrmg_da_lmer_filt_maaslin_all.csv")
#filt_maaslin_interaction <- read.csv("data/da_lmer_filt_maaslin_interaction.csv")
filt_fit_data_bal_v <- read.csv("Git/Host_depletion_macmini/data/mrkrmg_da_lmer_filt_fit_data_bal.csv")
filt_fit_data_spt_v <- read.csv("Git/Host_depletion_macmini/data/mrkrmg_da_lmer_filt_fit_data_spt.csv")
filt_fit_data_ns_v <- read.csv("Git/Host_depletion_macmini/data/mrkrmg_da_lmer_filt_fit_data_ns.csv")
...
filt_maaslin_all_v %>% subset(., .$qval < 0.1 ) %>% .$metadata %>% table
...
filt_maaslin_all_v_h <- read.csv("Git/Host_depletion_macmini/data/mrkrmg_host_da_lmer_filt_maaslin_all.csv")
#filt_maaslin_interaction <- read.csv("data/da_lmer_filt_maaslin_interaction.csv")
filt_fit_data_bal_v_h <- read.csv("Git/Host_depletion_macmini/data/mrkrmg_host_da_lmer_filt_fit_data_bal.csv")
filt_fit_data_spt_v_h <- read.csv("Git/Host_depletion_macmini/data/mrkrmg_host_da_lmer_filt_fit_data_spt.csv")
filt_fit_data_ns_v_h <- read.csv("Git/Host_depletion_macmini/data/mrkrmg_host_da_lmer_filt_fit_data_ns.csv")
#filt_fit_data_pos <- read.csv("data/da_lmer_filt_fit_data_pos.csv")
...
#Baloon plot
        ...
        #ylab("Species") +
        labs(tag = "B", title = "BAL (viral host)")  +
...
#Baloon plot
        ...
        #xlab("Experimental group") +
        ylab("Viral host genus") +
        labs(tag = "D", title = "Nasal swab (viral host)")  +
        #ggtitle("D Nasal swab (viral host)") +
        theme(panel.grid.major = element_line(colour = "grey"),
              legend.position = "top",
              plot.title = element_text(hjust = 1),
              plot.tag.position = c(0.05, 0.95),
...
#Baloon plot
        ...
        #xlab("Experimental group") +
        #ylab("Species") +
        labs(tag = "F", title = "Sputum (viral host)")  +
        #ggtitle("F  Sputum (viral host)") +
        theme(panel.grid.major = element_line(colour = "grey"),
              legend.position = "top",
              plot.title = element_text(hjust = 1),
              plot.tag.position = c(0.05, 0.95),
              axis.text.x = element_text(angle = 45, hjust=1, color = c("#e31a1c", "#fb9a99", "#33a02c", "#b2df8a", "#1f78b4", "#a6cee3")),
              #Element markdown for taxa name italicizing
              axis.text.y = ggtext::element_markdown(size = 8),
              axis.title.y = element_blank(),
              axis.title.x = element_blank(),
        ...
        #scale_x_discrete(breaks = c("Untreated", "lyPMA", "Benzonase", "HostZERO", "MolYsis", "QIAamp"),
        #        labels=c("U", "L", "B", "H", "M","Q")
        #                 )
...
DA_vh_all <- filt_maaslin_all_v_h %>% subset(., .$metadata != "sample_type") %>% subset(., .$qval < 0.1)  %>% .$feature %>% unique
DA_vh_all <- filt_maaslin_all_v_h %>% subset(., .$metadata != "sample_type") %>% subset(., .$qval < 0.1 & abs(.$Estimate)> 0.3)  %>% .$feature %>% unique

DA_vh_strtfd <- c(
        filt_fit_data_bal_v_h %>% subset(., .$metadata != "sample_type") %>% subset(., .$qval < 0.1)  %>% .$feature %>% unique,
        filt_fit_data_ns_v_h %>% subset(., .$metadata != "sample_type") %>% subset(., .$qval < 0.1)  %>% .$feature %>% unique,
        filt_fit_data_spt_v_h %>% subset(., .$metadata != "sample_type") %>% subset(., .$qval < 0.1)  %>% .$feature %>% unique
)  %>% unique

```

## Microbial - viral host correlation
...
filt_maaslin_all <- read.csv("Git/Host_depletion_macmini/data/da_lmer_filt_maaslin_all.csv")

#filt_maaslin_interaction <- read.csv("data/da_lmer_filt_maaslin_interaction.csv")
filt_fit_data_bal_m <- read.csv("Git/Host_depletion_macmini/data/da_lmer_filt_fit_data_bal.csv")
filt_fit_data_spt_m <- read.csv("Git/Host_depletion_macmini/data/da_lmer_filt_fit_data_spt.csv")
filt_fit_data_ns_m <- read.csv("Git/Host_depletion_macmini/data/da_lmer_filt_fit_data_ns.csv")
#filt_fit_data_pos <- read.csv("data/da_lmer_filt_fit_data_pos.csv")
#Stratified by sample type
...
### bacterial species - viral host genus without filtering

All data without p-value based filtering
...
### bacterial species - viral host genus

p-Bonferroni < 0.5 removed (noted was white) data
...
Data with p-Bonferroni filtering
...
#        scale_x_discrete(drop=) +
        facet_wrap(~ treatment,  nrow = 1, drop = T, scales = "free_x") +
        ggtitle("Nasal swabs (microbial species)") +
        labs(tag = "C")
...
e <- tax_table(subset_samples(phyloseq$phyloseq_rel,
   ...
   phyloseq_temp
        ...
        facet_wrap(~ treatment, scales= "free_x", nrow = 1) +
        ggtitle("Sputum (microbial species)") +
        labs(tag = "E")
...
tiff(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/Figure2_updated.tiff",   # The directory you want to save the file in
...
pdf(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/Figure2_updated.pdf",   # The directory you want to save the file in
...
c <- tax_table(subset_samples(phyloseq$phyloseq_rel,
   ...
   phyloseq_temp
        ...
        my_plot_bar(., x = "treatment", fill="species20") + 
        ylab("") +
        theme_classic(base_size = 11, base_family = "sans") +
        theme(legend.text = element_markdown(size = 6),
              legend.key.size = unit(3, "mm"),
...
d <- tax_table(subset_samples(phyloseq$phyloseq_rel,
   ...
   phyloseq_temp
        ...
        my_plot_bar(., x = "treatment_order", fill="species20") + 
        ylab("") +
        theme_classic(base_size = 11, base_family = "sans") +
        theme(legend.text = element_markdown(size = 6),
              legend.key.size = unit(3, "mm"),
...
#        scale_x_discrete(drop=) +
        facet_grid(~ subject_id, drop = T, scales = "free", space = "free") +
...
e <- tax_table(subset_samples(phyloseq$phyloseq_rel,
   ...
   phyloseq_temp
        ...
        facet_wrap(~ subject_id, scales= "free_x", nrow = 1) +
        #scale_x_discrete(breaks = c("Untreated", "lyPMA", "Benzonase", "HostZERO", "MolYsis", "QIAamp"),
        #                 labels = c("U", "L", "B", "H", "M", "Q")) +
        ggtitle("Sputum (microbial species)") +
        labs(tag = "E")
...
c_vh <- tax_table(subset_samples(v_h_phyloseq_rel_renamed,
   ...
   phyloseq_temp
        ...
        theme(legend.text = element_markdown(size = 6),
              legend.key.size = unit(3, "mm"),
              legend.title = element_text(size = 6),
              axis.text.x = element_text(angle = 45, hjust=1, size = 7, color = c("#e31a1c", "#fb9a99", "#33a02c", "#b2df8a", "#1f78b4", "#a6cee3")),
              plot.tag.position = c(0.03, 0.97),
              axis.title.x = element_blank()) +
        guides(fill=guide_legend(title="Top 10 viral host at genus level", ncol = 2)) +
        scale_fill_manual(values = c('#ccebc5',# Bacteroides
...
d_vh <- ggplotGrob(d_vh + facet_grid(~ subject_id, drop = T, scales = "free", space = "free"))
#https://stackoverflow.com/questions/45843759/ggplot2-coloring-axis-text-on-a-faceted-plot
#d_vh$grobs[[22]]$children[[2]]$grobs[[2]]$children[[1]]$gp$col <- c("#e31a1c", "#fb9a99", "#33a02c", "#b2df8a", "#1f78b4", "#a6cee3")

#Customizing text colors of d_

d_vh$grobs[[22]]$children[[2]]$grobs[[2]]$children[[1]]$label
d_vh$grobs[[23]]$children[[2]]$grobs[[2]]$children[[1]]$label
d_vh$grobs[[22]]$children[[2]]$grobs[[2]]$children[[1]]$gp$col <- c("#e31a1c", "#fb9a99", "#a6cee3")
d_vh$grobs[[23]]$children[[2]]$grobs[[2]]$children[[1]]$gp$col <- c("#e31a1c", "#fb9a99", "#a6cee3")

d_vh$grobs[[24]]$children[[2]]$grobs[[2]]$children[[1]]$label
d_vh$grobs[[25]]$children[[2]]$grobs[[2]]$children[[1]]$label
d_vh$grobs[[26]]$children[[2]]$grobs[[2]]$children[[1]]$label
d_vh$grobs[[27]]$children[[2]]$grobs[[2]]$children[[1]]$label
d_vh$grobs[[24]]$children[[2]]$grobs[[2]]$children[[1]]$gp$col <- c("#e31a1c", "#33a02c", "#b2df8a", "#1f78b4")
d_vh$grobs[[25]]$children[[2]]$grobs[[2]]$children[[1]]$gp$col <- c("#e31a1c", "#33a02c", "#b2df8a", "#1f78b4")
d_vh$grobs[[26]]$children[[2]]$grobs[[2]]$children[[1]]$gp$col <- c("#e31a1c", "#33a02c", "#b2df8a", "#1f78b4")
d_vh$grobs[[27]]$children[[2]]$grobs[[2]]$children[[1]]$gp$col <- c("#e31a1c", "#33a02c", "#b2df8a", "#1f78b4")



d_vh$grobs[[28]]$children[[2]]$grobs[[2]]$children[[1]]$label
d_vh$grobs[[28]]$children[[2]]$grobs[[2]]$children[[1]]$gp$col <- c("#e31a1c","#fb9a99", "#33a02c", "#b2df8a")

d_vh$grobs[[29]]$children[[2]]$grobs[[2]]$children[[1]]$label
d_vh$grobs[[29]]$children[[2]]$grobs[[2]]$children[[1]]$gp$col <- c("#e31a1c", "#1f78b4", "#a6cee3")

d_vh$grobs[[30]]$children[[2]]$grobs[[2]]$children[[1]]$label
d_vh$grobs[[31]]$children[[2]]$grobs[[2]]$children[[1]]$label
d_vh$grobs[[30]]$children[[2]]$grobs[[2]]$children[[1]]$gp$col <- c("#e31a1c", "#fb9a99", "#a6cee3")
d_vh$grobs[[31]]$children[[2]]$grobs[[2]]$children[[1]]$gp$col <- c("#e31a1c", "#fb9a99", "#a6cee3")



d_vh_legend <- tax_table(subset_samples(v_h_phyloseq_rel_renamed,
                              sample_type == "Nasal")) %>%
        cbind(species20 = "Other") %>%
        {top20species <- head(taxa_sums(subset_samples(v_h_phyloseq_rel_renamed,
                              sample_type == "Nasal")) %>% 
...
tiff(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/Figure2_updated_v2.tiff",   # The directory you want to save the file in
    width = 170, # The width of the plot in inches
    height = 225, # The height of the plot in inches
    units = "mm",
    res = 250
...
fig2_updated <- ggarrange(c, ggarrange(ggplot() + theme_void(), c %>% lemon::g_legend() %>% as_ggplot, ncol = 1, heights = c(1, 4)),
                  c_vh, c_vh %>% lemon::g_legend() %>% as_ggplot,
                  d, d %>% lemon::g_legend() %>% as_ggplot,
                  d_vh %>% as_ggplot, d_vh_legend %>% lemon::g_legend() %>% as_ggplot,
                  e, e %>% lemon::g_legend() %>% as_ggplot,
                  e_vh, e_vh %>% lemon::g_legend() %>% as_ggplot,
                  #a, a %>% lemon::g_legend() %>% as_ggplot,
                  #b, b %>% lemon::g_legend() %>% as_ggplot,
                  ncol=2,nrow=6,
                  widths = c(3, 1), heights = c(2, 2.72, 2, 2.72, 2, 2.72),
                  legend = "none",
                  align = "hv")
...
pdf(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/Figure2_updated_v2.pdf",   # The directory you want to save the file in
    width = 183 / 25.4, # Convert width from mm to inches (180 mm)
    height = 225 / 25.4, # Convert height from mm to inches (160 mm)
    paper = "special",   # Prevents default paper size settings
    onefile = FALSE      # Ensures a single page per PDF file
)
...
f3a <- ggplot(subset(sample_data(phyloseq$phyloseq_count) %>% 
        ...
        scale_color_manual(values = c("#e31a1c", "#fb9a99", "#33a02c", "#b2df8a", "#1f78b4", "#a6cee3"), name = "Treatment", labels = c("Untreated","lyPMA", "Benzonase", "HostZERO", "MolYsis", "QIAamp")) + #color using 
        scale_fill_manual(values = c("#e31a1c", "#fb9a99", "#33a02c", "#b2df8a", "#1f78b4", "#a6cee3"), name = "Treatment", labels = c("Untreated","lyPMA", "Benzonase", "HostZERO", "MolYsis", "QIAamp")) + #color using 
        ggtitle("Microbial alpha diversity") +
        labs(tag = "A") +
        theme(plot.tag = element_text(size = 15),
              plot.tag.position = c(0.03, 0.97),
              axis.text.x = element_blank(),
...
f3b2_box <- horn_dist_long_within_sampleid_from_control %>% 
  ...
  mutate(treatment = factor(treatment, levels = c("lypma", "benzonase", "host_zero", "molysis", "qiaamp"),
        ...
        theme(plot.tag = element_text(size = 15),
              ...
              legend.position = "none") +
        labs(tag = "C") +
        ggtitle("Microbial beta diversity") +
        geom_vline(xintercept = 0, col = "black", linetype="dotted") +
        #coord_cartesian(xlim=c(-0.5, 1)) +
        #geom_text(aes(x = 0, label = treatment), hjust = 0, nudge_x = -.55, size = 3, color = "black", family = "sans") +
        #geom_text(aes(x = 0, label = text), hjust = 0, nudge_x = -0.4, size = 3, color = "black", family = "sans") +
...
tiff(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/Figure3_updated.tiff",   # The directory you want to save the file in
    width = 170, # The width of the plot in inches
    height = 225, # The height of the plot in inches
    units = "mm",
    res = 250
...
figS7 <- filt_maaslin_all %>% mutate(feature = species_italic(feature),
        ...
        scale_color_manual(values = c("#a65628",
                           ...
                           labels = c("log<sub>10</sub>(Final reads)",
                                      ...
                                      "MolYsis",
                                      "QIAamp")) + #color using https://colorbrewer2.org/#type=qualitative&scheme=Set1&n=6
        guides(col = guide_legend(title = "Factors", title.position = "left", nrow = 1)) +
        ggrepel::geom_text_repel(aes(label = label1),
                                  point.padding = unit(0.5, "lines"),
                                  #nudge_y = 10, box.padding = unit(0.35, "lines"),
                                  na.rm = T,
...
filt_maaslin_all_v_h$group <-  ifelse(filt_maaslin_all_v_h$Estimate > 0.3 & filt_maaslin_all_v_h$qval < 0.1, "more",
                     ifelse(filt_maaslin_all_v_h$Estimate < -0.3 & filt_maaslin_all_v_h$qval < 0.1, "less",
                            "insignificant"))
...
figS7_host <- filt_maaslin_all_v_h %>% 
        ...
        guides(col = guide_legend(title = "Factors", title.position = "left", nrow = 1)) +
                ggrepel::geom_text_repel(aes(label = label1),
                                  ...
                                  grob = "richtext",
                                  show.legend = F,
                                  size=3, segment.size=0.25, nudge_x=5, 
                                  #nudge_y=1, 
                                  direction="y") +
        ggrepel::geom_text_repel(aes(label = label2),
                                   box.padding = unit(0.35, "lines"),
...
tiff(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/Figure4_updated.tiff",   # The directory you want to save the file in
    width = 170, # The width of the plot in inches
    height = 225, # The height of the plot in inches
    units = "mm",
    res = 250
...
pdf(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/Figure4_updated.pdf",   # The directory you want to save the file in
    width = 183 / 25.4, # Convert width from mm to inches (180 mm)
    height = 220 / 25.4, # Convert height from mm to inches (160 mm)
    paper = "special",   # Prevents default paper size settings
    onefile = FALSE      # Ensures a single page per PDF file
...
#filt_fit_data_pos <- make_sig_table(filt_fit_data_pos)
filt_fit_data_bal <- make_sig_table(filt_fit_data_bal_m)
filt_fit_data_ns <- make_sig_table(filt_fit_data_ns_m)
filt_fit_data_spt <- make_sig_table(filt_fit_data_spt_m)
...
#Baloon plot
        ...
        gradient_fill(c("#006d2c", "#edf8fb")) +
        xlab("Experimental group") +
        ylab("Microbial species") +
        #labs(tag = "C")  +
        labs(tag = "C", title = "Nasal swab (microbe)")  +
...
#Baloon plot
        ...
        #ylab("Species") +
        #labs(tag = "E")  +
        labs(tag = "E", title = "Sputum (microbe)")  +
...
fig4 <- ggarrange(f5c %>% lemon::g_legend() %>% as_ggplot,
                  ggarrange(
                          ggarrange(f5b, f5c, f5d, legend = "none", ncol = 1, align = "v", heights = c(4, 4, 5.2)),
                          ggarrange(f5b_host, f5c_host, f5d_host, legend = "none", ncol = 1, align = "v", heights = c(4, 4, 5.2)),
...
annotate_figure(fig4,
                bottom = text_grob("Treatment method",
                                 rot = 0,
                                 family = "sans", 
                                 size = 11))

tiff(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/FigureS8_updated.tiff",   # The directory you want to save the file in
    width = 170, # The width of the plot in inches
    height = 225, # The height of the plot in inches
    units = "mm",
    res = 250
...
pdf(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/Figure5_updated.pdf",   # The directory you want to save the file in
    width = 183 / 25.4, # Convert width from mm to inches (180 mm)
    height = 220 / 25.4, # Convert height from mm to inches (160 mm)
    paper = "special",   # Prevents default paper size settings
    onefile = FALSE      # Ensures a single page per PDF file
...
### New figure (correlation plot)

### bacterial species - viral host genus

list of DA microbial species

```{r}

DA_m_all <- filt_maaslin_all %>% subset(., .$metadata != "sample_type") %>% subset(., .$qval < 0.01 & abs(Estimate) > 2)  %>% .$feature %>% unique
        
        
DA_m_strtfd <- c(
        filt_fit_data_bal_m %>% subset(., .$metadata != "sample_type") %>% subset(., .$qval < 0.01 & abs(Estimate) > 2)  %>% .$feature %>% unique,
        filt_fit_data_ns_m %>% subset(., .$metadata != "sample_type") %>% subset(., .$qval < 0.01 & abs(Estimate) > 2)  %>% .$feature %>% unique,
        filt_fit_data_spt_m %>% subset(., .$metadata != "sample_type") %>% subset(., .$qval < 0.01 & abs(Estimate) > 2)  %>% .$feature %>% unique
)  %>% unique 

```

```{r}
#OTU table of microbial output, subsetted by DA taxa

otu_bac <- phyloseq$phyloseq_rel %>%
        subset_taxa(., Species %in% paste0("s__", DA_m_all)) %>% 
        otu_table(., taxa_are_rows = F) %>%
        data.frame(., check.names = F) 
...
taxa_names(v_h_phyloseq_rel) <- v_h_phyloseq_rel %>%
        ...
        data.frame %>% 
        .$Genus
tax_table(v_h_phyloseq_rel)
#OTU table of viral output
otu_v_h <- v_h_phyloseq_rel %>%
        subset_taxa(., Genus %in% DA_vh_all) %>% 
        otu_table(., taxa_are_rows = F) %>%
        data.frame(., check.names = F)
...
flat_corr <- flattenCorrMatrix(spearman_corr$r, spearman_corr$P) %>%
        ...
        subset(., .$Var1 %in% taxa_names(v_h_phyloseq_rel)) %>%
        subset(., !(.$Var2 %in% taxa_names(v_h_phyloseq_rel))) %>%
        mutate(r_sig = case_when(p_b > 0.05 ~ 0,
                             .default = r)) # bonferroni & 0.001 criteria was used as their correction was the strongest
...
heatmap_corr_sig <- flat_corr %>% select(c("Var1", "Var2", "r_sig")) %>%
        spread(., Var2, r_sig) %>% 
        column_to_rownames("Var1") %>%
        #filter(all_vars(!is.na(.))) %>%
        as.matrix(.) 
...
ComplexHeatmap::Heatmap(heatmap_corr, 
        ...
        column_dend_side = "bottom",
        column_names_side = "top",
        row_title = "Viral host genus",
        column_title =  "Microbial species",
        clustering_method_rows = "average",
        clustering_method_columns = "average",
        col=circlize::colorRamp2(c(-1, 0, 1), c("navy", "white", "red")),
        heatmap_legend_param = list(title = "Spearman correlation coefficient (rho)",  title_position = "leftcenter-rot"),
        cell_fun = function(j, i, x, y, width, height, fill) {
          grid::grid.text(sprintf("%.2f", heatmap_corr[i, j]), x, y, gp =
                      grid::gpar(fontsize = 7, col="white"))})


ComplexHeatmap::Heatmap(heatmap_corr_sig, 
                        column_order = colnames(heatmap_corr) %>% order,
                        row_order = rownames(heatmap_corr) %>% order,
                        na_col = "grey",
        rect_gp = grid::gpar(col = "grey"),
...
col_labels <- colnames(heatmap_corr) %>% 
        #.[order(.)] %>% 
        species_italic()
...
row_labels <- rownames(heatmap_corr) %>%
        #.[order(.)] %>% 
        species_italic()



tiff(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/FigureS15.tiff",   # The directory you want to save the file in
    width = 170, # The width of the plot in inches
    height = 170, # The height of the plot in inches
    units = "mm",
    res = 250
...
ComplexHeatmap::Heatmap(heatmap_corr, 
        ...
        row_names_gp = grid::gpar(fontsize = 10),
        column_names_gp = grid::gpar(fontsize = 10),
        column_labels = ComplexHeatmap::gt_render(col_labels),
        row_labels = ComplexHeatmap::gt_render(row_labels),
        column_names_rot = 45,
        column_dend_side = "bottom",
        column_names_side = "top",
        row_title = "Viral host genus",
