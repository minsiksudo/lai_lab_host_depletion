...
contaminants <- rbind(contaminant_combined_lypma,
                      contaminant_combined_ben, 
                      contaminant_combined_hostzero,
                      contaminant_combined_molysis,
                      contaminant_combined_QIAamp
...
names(contaminants) <- c("Treatment", "Taxa")

merged_contaminants_treatment <- merge(contaminants, prev_all %>% rownames_to_column("Taxa"), by = "Taxa") %>%
        merge(., prev_neg %>% rownames_to_column("Taxa"), by = "Taxa") %>%
       dplyr::select(c("Treatment", "Taxa", "Prevalence (all)", "Prevalence (negative controls)")) %>%
        .[order(.$"Treatment", .$"Taxa"),] %>%
        remove_rownames()
