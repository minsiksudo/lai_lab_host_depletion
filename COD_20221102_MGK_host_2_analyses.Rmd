...
qPCR_results <- read_csv("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Tables/DAT_20230122_MGK_host_control_qPCR.csv")
...
2. QC
        ...
        Q2. How were the extraction controls
        Q3. Decontam - were there any contaminants?
...
hist((log10(sample_data$Final_reads + 1)),100) # no somaple showed 0 reads
hist((log10((phyloseq %>% otu_table %>% colSums()) + 1)),100, main = "Histogram of total reads (sum of OTU table)") # 2 samples showed 0 total reads (sum of otu_table)

#how were the samples failed in library prep?
sample_data %>% data.frame %>% mutate(total_read = phyloseq %>% otu_table %>% colSums()) %>% rownames_to_column(var = "baylor_other_id") %>%
...
sample_data %>% data.frame %>% filter(phyloseq %>% otu_table %>% colSums() == 0) # two BAL sampels showed 0 total reads
sample_data(phyloseq) %>% data.frame() %>% subset(., .$lib_failed)

```
*Results:*

*1.1. 13 samples failed in library prep*

*1.2. Two BAL sampels showed 0 total reads*

*1.3. Sequencing fail ≠ library prep fail*
...
# Q2. How was the extraction controls, both pos and neg?#
...
zymo_mock <- read_excel("/Users/minsikkim/Dropbox (Partners HealthCare)/@minsik/project_sicas2/data_raw/DAR_20210929_zymo_mock_data.xlsx") %>%
        data.frame(row.names = T) %>% rename(mock_theoretical = Mock) %>%
        phyloseq(otu_table(., taxa_are_rows = T), tax_table(phyloseq))

phyloseq_control_rel <- rbind(c("mock_theoretical", "mock")) %>% data.frame() %>% column_to_rownames(var = "X1") %>% rename(sample_type = X2) %>%
        merge_phyloseq(sample_data(.), zymo_mock) %>%        #making sample_data of mock community
        merge_phyloseq(., subset_samples(phyloseq, sample_type == "pos_extraction" | sample_type == "neg_extraction" )) %>% #adding data of controls
        transform_sample_counts(., function(x){x / sum(x)})

#Species richness of each control groups
S.obs <- rowSums(t(otu_table(phyloseq_control_rel)) != 0)
...
#Since species level identifiers can differ by annotation methos, Genus level was analyzed
phyloseq_control_rel_genus <- phyloseq::tax_glom(phyloseq_control_rel, "Genus", NArm = TRUE)
S.obs <- rowSums(t(otu_table(phyloseq_control_rel_genus)) != 0)

sample_data(phyloseq_control_rel_genus)$detail <- c("Mock, 10 Genus", "Pos., 28 Genus", "Neg., 3 Genus")
sample_data(phyloseq_control_rel_genus)$detail <- factor(sample_data(phyloseq_control_rel_genus)$detail, levels = c("Mock, 10 Genus", "Pos., 28 Genus", "Neg., 3 Genus"))

plot_bar(phyloseq_control_rel_genus, fill="Genus") + 
        ylab("Relative abundancne") +
        theme_classic(base_size = 11, base_family = "serif") +
        ggtitle("Genus richness of control data") +
        theme(legend.position = "none") +
        facet_wrap (~ detail, scales= "free_x", nrow=1)
...
```
Results
        2.1. Negative control showed minimal number of possible contaminants
        2.2. Positive control contained various contaminants
