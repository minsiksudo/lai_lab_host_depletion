---
title: "COD_20230831_HOST_4_analysis_figure_table_with_output"
author: "Minsik Kim"
date: "2023-08-31"
output:
    rmdformats::downcute:
...
ctab_bal <- cbind(OR=fixef(glmer_libfail_bal), cc)
exp(ctab_bal)
...
ctab_ns <- cbind(OR=fixef(glmer_libfail_nasal), cc)
exp(ctab_ns)
...
**There is no lib failed samples with sputum. Therefore we cannot run the analysis**

```{r}

glmer_libfail_sputum <- glmer(lib_failed ~ treatment + (1|subject_id), data = sample_data %>% data.frame %>% dplyr::filter(sample_type %in% c("Sputum")))

#cc <- confint(glmer_libfail_sputum,parm="beta_")  ## slow (~ 11 seconds)
#ctab_spt <- cbind(OR=fixef(glmer_libfail_sputum), cc)
#exp(ctab_spt)
