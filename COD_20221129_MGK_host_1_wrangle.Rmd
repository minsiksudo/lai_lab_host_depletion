...
#Contorl data
phyloseq2 <- microbiome::read_biom2phyloseq("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_Baylor/4_Data/1_Raw/Baylor_Processed/")
#Merging
phyloseq <- merge_phyloseq(phyloseq, phyloseq2)
...
#control data
readstat2 <- read.csv("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_Baylor/4_Data/1_Raw/Baylor_Processed", sep = "\t") %>%
        rename(baylor_other_id = SampleID)
#Merging
...
setwd("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_Baylor/4_Data/1_Raw/Baylor_Processed/20221102_PQ00331_Host_Depletion/humann3")
path_rpk <- read.csv("PathAbundance.relab.metagenome.txt", sep = "\t", check.names = F, row.names = "# Pathway") %>% select(-c("taxonomy"))
path_tax <- read.csv("PathAbundance.relab.metagenome.txt", sep = "\t", check.names = F) %>%
        select(c("taxonomy", "# Pathway")) %>% rename(pathway = "# Pathway", group = "taxonomy")
row.names(path_tax) <- path_tax$pathway

setwd("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_Baylor/4_Data/1_Raw/Baylor_Processed/20230401")
path_rpk2 <- read.csv("humann3/PathAbundance.relab.metagenome.txt", sep = "\t", check.names = F, row.names = "# Pathway") %>% select(-c("taxonomy"))
path_tax2 <- read.csv("humann3/PathAbundance.relab.metagenome.txt", sep = "\t", check.names = F) %>%
        select(c("taxonomy", "# Pathway")) %>% rename(pathway = "# Pathway", group = "taxonomy")

phy <- list()
phy$phyloseq_count <- phyloseq
phy$phyloseq_rel <- transform_sample_counts(phyloseq, function(x){x / sum(x)})
phy$phyloseq_path_rpkm <- merge_phyloseq(
