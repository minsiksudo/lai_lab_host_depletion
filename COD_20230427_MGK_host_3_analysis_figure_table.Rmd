...
tinyvamp_untreated <- read_rds("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/4_Data/2_Tidy/HOST_tinyvamp_decontaminated_Amy_Willis/20230719_amy/untreated_sparser2.RDS") %>%
        t()

tinyvamp_lypma <- read_rds("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/4_Data/2_Tidy/HOST_tinyvamp_decontaminated_Amy_Willis/20230719_amy/lypma_sparser2.RDS") %>%
        t()

tinyvamp_benzonase <- read_rds("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/4_Data/2_Tidy/HOST_tinyvamp_decontaminated_Amy_Willis/20230719_amy/benzonase_sparser2.RDS") %>%
        t()

tinyvamp_host_zero <- read_rds("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/4_Data/2_Tidy/HOST_tinyvamp_decontaminated_Amy_Willis/20230719_amy/hostzero_sparser2.RDS") %>%
        t()

tinyvamp_molysis <- read_rds("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/4_Data/2_Tidy/HOST_tinyvamp_decontaminated_Amy_Willis/20230719_amy/molysis_sparser2.RDS") %>% 
        t()

tinyvamp_qiaamp <- read_rds("/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/4_Data/2_Tidy/HOST_tinyvamp_decontaminated_Amy_Willis/20230719_amy/qiaamp_sparser2.RDS") %>% 
        t()
...
sample_data_tv <- phyloseq$phyloseq_rel %>%
...
figS1_final_reads <- sample_data %>% data.frame %>% mutate(total_read = phyloseq$phyloseq_count %>% otu_table %>% colSums()) %>%
...
figS1_total_reads <- sample_data %>% data.frame %>% mutate(total_read = phyloseq$phyloseq_count %>% otu_table %>% colSums()) %>%
...
### Bar plot of negative controls (Fig. S5)

**Fig. S5**. Barplot of negative controls by treatment methods at species level

### {-}


## decontam (Table S4) {.tabset}

### decontam - stratified by sample_type (Table S4)


**Stratified analysis** showed **no contaminants** in **NS** and **BAL**

**Table S4. ** Summary table of potential contaminants with stratified decontam (David et al., 2018) with combined method and prevalence of each taxa at all samples and in negative controls.
...
species_italic <- function(data) {
  ...
  names <- gsub(" sp.", "</i> sp.", names)
  names <- gsub(" group", "", names)
  names <- ifelse(grepl("</i>", names), paste("<i>", names, sep = ""), paste("<i>", names, "</i>", sep = ""))
  names
}
        ...
        data.frame(function. = function_qc$function.,
                   red_flag_prev_abd = 
                           ifelse(function_qc$prevalence < 
                                          otu_table(phyloseq$phyloseq_path_rpk) %>%
                                          t %>% 
                                          rownames() %>%
...
phyloseq$phyloseq_rel <- transform_sample_counts(phyloseq$phyloseq_count, function(x){x/sum(x)})
...
table1 <- sample_data(phyloseq$phyloseq_count_prev) %>% data.frame() %>% 
        ...
        summarise(`Sample number<br>N` = n(),
...
## **+ α** *Amy's analys result have changes in taxa?*
...
figS5 <- ggarrange(c, d, e, a, b, ncol = 1, common.legend = T, legend = "top", aline = "hv")
...
tableS5 <- cbind(tableS5_mock, tableS5_bal, tableS5_ns, tableS5_spt) %>%
        kbl(format = "html", escape = 0) %>% kable_styling(full_width = 0, html_font = "serif") %>% 
...
Effect size, standard error (SE) and p-value of a statistical test for species richness using linear mixed effect model stratified by sample type (Species richness ~ treatment + (1|subject_id) ).
...
cbind(sr_lmer_mock, sr_lmer_bal, sr_lmer_ns, sr_lmer_spt) %>%
    kbl(format = "html", escape = 0) %>%
        add_header_above(c(" " = 1, "Mock" = 3, "BAL" = 3, "Nasal swab" = 3,"Sputum"= 3)) %>% 
        kable_styling(full_width = 0, html_font = "serif")
...
### PERMANOVA - all samples (Table S6)

**Table S6. ** Degree of freedom, effect size (residual, R^2) and p-value of permutational ANOVA for Morisita-Horn distiances with an interaction term and strata term (BC-distance ~ sample_type * treatment + log10 (Final_reads), strata = subject_id).
...
tableS6 <- horn_perm_all %>% data.frame(check.names = F) %>% rownames_to_column("row.names") %>% 
...
### PERMANOVA - Stratified (Table S7)

**Table S7**. Degree of freedom, effect size (residual, R^2) and p-value of permutational ANOVA for Morisita-Horn distiances  for species richness stratified by sample type (MH-distance ~ lyPMA + Benzoase + Host zero + Molysis + QIAamp + log10 (Final_reads), strata = subject_id).
...
save_kable(table3, file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/tableS7.html", self_contained = T)


```

### LM on M-H distance (Table S8)

**Table S8**. Effect size, standard error (SE) and p-value of a statistical test for Horn-Morisita distance from untreated to each treated within subject (BC-distance wihtin subject by treatment~ sample type + treatment + sampetype * treatment + log10 (Final_reads) + (1|subject_id) ).
...
### List of DA taxa (Table S9)

**Table S9.** List of differentailly abundant taxa identified by MaAsLin. Significant associations (q < 0.1) that is showing high change (|log2-fold change| > 0.5) were listed.

```{r, warning=FALSE, message=FALSE}

tableS9 <- filt_maaslin_all %>% subset(., .$qval < 0.1 & abs(.$coef) > 0.5) %>% subset(., .$metadata != "sample_type" & .$metadata != "log10.Final_reads") %>% select(c("feature", "metadata", "coef", "qval")) %>%
...
### Function richness - all & stratified (Table S10)

**Table S10.** Effect size, standard error (SE) and p-value of a statistical test for function richness with an interaction term using linear mixed effect model (Species richness ~ treatment + (1|subject_id) ).
...
tables10 <- cbind(sr_lmer_mock, sr_lmer_bal, sr_lmer_ns, sr_lmer_spt) %>%
    kbl(format = "html", escape = 0) %>%
        add_header_above(c(" " = 1, "All" = 3, "BAL" = 3, "Nasal swab" = 3,"Sputum"= 3)) %>% 
        kable_styling(full_width = 0, html_font = "serif")
...
### Function beta - stratified (Table S11)
...
tables11
save_kable(tables10, file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/tableS11.html", self_contained = T)
...
f10b <- ggplot(subset(sample_data(phyloseq_tv) %>% 
...
figa1 <- ggarrange(f10a, f10b, common.legend = T, ncol = 1)
...
### Species richness change by treatment (after decontamination)
...
figd2a <- ordinate(subset_samples(phyloseq_rel_nz, sample_type != "Neg." & sample_type != "Mock"), method = "PCoA", distance = "horn") %>%
        ...
        facet_wrap(~sample_type, scales = "free") +
        labs(tag = "A") +
        ggtitle("Raw microbial composition") +
        theme(plot.tag = element_text(size = 15), legend.position = "top")# +
        #stat_ellipse(type = "norm") +
        #stat_ellipse(type = "t")

figd2b <- ordinate(subset_samples(phyloseq_tv, sample_type != "Neg." & sample_type != "Mock"), method = "PCoA", distance = "horn") %>%
        ...
        facet_wrap(~sample_type, scales = "free") +
        labs(tag = "B") +
        ggtitle("Decontaminated microbial composition") +
        theme(plot.tag = element_text(size = 15), legend.position = "top")# +
        #stat_ellipse(type = "norm") +
        #stat_ellipse(type = "t")


figA3 <- ggarrange(figd2a, figd2b, common.legend = T, nrow = 2)
figA3
...
### Differences between raw vs decontaminated - PCA plot

```{r}
sample_data(phyloseq_tv)$decontaminated <- 1

sample_data(phyloseq$phyloseq_rel)$decontaminated <- 0

phyloseq_tv_compare <- merge_phyloseq(phyloseq_tv,
                                       phyloseq$phyloseq_rel) %>% 
                                subset_samples(., S.obs != 0)

sample_data(phyloseq_tv_compare)$decontaminated <-
        as.factor(sample_data(phyloseq_tv_compare)$decontaminated)

figA4 <- ordinate(subset_samples(phyloseq_tv_compare,
                        sample_type != "Neg." & sample_type != "Mock"),
         method = "PCoA", distance = "horn") %>%
        plot_ordination(phyloseq_tv_compare,
                        .,
                        col = "treatment", shape = "decontaminated") +
        #scale_color_viridis(discrete = 6, name = "Treatment", labels = c("Mock theoretical", "Control","lyPMA", "Benzonase", "Host zero", "Molysis", "QIAaamp")) +
        scale_color_manual(values = c("#e31a1c", "#fb9a99", "#33a02c", "#b2df8a", "#1f78b4", "#a6cee3"),
                           name = "Treatment",
                           breaks = c("Untreated","lyPMA", "Benzonase", "Host zero", "Molysis", "QIAamp"),
        ...
        ggtitle("Decontaminated microbial composition") +
        theme(plot.tag = element_text(size = 15), legend.position = "top") +
        guides(shape = guide_legend(title = "Decontaminated"))
        #stat_ellipse(type = "norm") +
        #stat_ellipse(type = "t")
...
png(file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/FigureA4.png",   # The directory you want to save the file in
    width = 180, # The width of the plot in inches
    height = 180, # The height of the plot in inches
    units = "mm",
    res = 600
...
### Differences between raw vs decontaminated - forest plot

```{r}

#Making subset of non-zero samples without neg

phyloseq_rel_nz <- phyloseq_tv_compare %>%
        subset_samples(S.obs != 0 & sample_type %in% c("BAL", "Nasal", "Sputum"))

sample_names(phyloseq_rel_nz) <- paste(
        sample_data(phyloseq_rel_nz)$original_sample,
        sample_data(phyloseq_rel_nz)$treatment,
        "_",
        sample_data(phyloseq_rel_nz)$decontaminated,
        sep = "_")
...
#Adding sample type and treatment name. 
#this can be also done by merging metadata into the `horn_dist_long`
horn_dist_long$sample_id_1 <- gsub("___1|___0", "", horn_dist_long$iso1)
horn_dist_long$sample_id_2 <- gsub("___1|___0", "", horn_dist_long$iso2)

horn_dist_long$method_1 <- ifelse(grepl("lyPMA", horn_dist_long$iso1, ignore.case = T),"lypma", 
                                         ifelse(grepl("ben", horn_dist_long$iso1, ignore.case = T),"benzonase", 
                                                ifelse(grepl("host", horn_dist_long$iso1, ignore.case = T),"host_zero", 
                                                       ifelse(grepl("qia", horn_dist_long$iso1, ignore.case = T),"qiaamp", 
                                                              ifelse(grepl("moly", horn_dist_long$iso1, ignore.case = T),"molysis", 
                                                                     "control")))))


#Adding data for iso 2 also should be done

horn_dist_long$method_2 <- ifelse(grepl("lyPMA", horn_dist_long$iso2, ignore.case = T),"lypma", 
                                         ifelse(grepl("ben", horn_dist_long$iso2, ignore.case = T),"benzonase", 
                                                ifelse(grepl("host", horn_dist_long$iso2, ignore.case = T),"host_zero", 
                                                       ifelse(grepl("qia", horn_dist_long$iso2, ignore.case = T),"qiaamp", 
                                                              ifelse(grepl("moly", horn_dist_long$iso2, ignore.case = T),"molysis", 
                                                                     "control")))))
...
#Making figure of beta diversity distances
fA5 <- path_horn_dist_long_within_sampleid_from_control %>% 
        mutate(across(sample_type, factor, levels=c("BAL", "Nasal","Sputum"))) %>%
        subset(., .$sample_type != "Neg.") %>% 
        group_by(sample_type, treatment) %>%
        summarise(mean = mean(dist, na.rm = TRUE),
  ...
  mutate(se = sd / sqrt(n),
         lower.ci = mean - qt(1 - (0.05 / 2), n - 1) * se,
         upper.ci = mean + qt(1 - (0.05 / 2), n - 1) * se,
         treatment = factor(treatment, levels = c("control", "lypma", "benzonase", "host_zero", "molysis", "qiaamp"),
                            labels = c("Untreated", "lyPMA", "Benzonase", "Host zero", "Molysis", "QIAaamp"))) %>%#,
         #text = paste(sprintf("%.2f", round(mean, digits = 2)), " [", sprintf("%.2f", round(lower.ci, digits = 2)), ", ", sprintf("%.2f", round(upper.ci, digits = 2)), "]", sep = "")) %>%
        ggplot(aes(x = mean, y = treatment, col = treatment)) +
        geom_point(aes(x=mean), shape=15, size=3) +
        ...
        scale_y_discrete(limits=rev) +
        scale_color_manual(values = c("#e31a1c", "#fb9a99", "#33a02c", "#b2df8a", "#1f78b4", "#a6cee3"), name = "Treatment", labels = c("Untreated", "lyPMA", "Benzonase", "Host zero", "Molysis", "QIAaamp")) + #color using https://colorbrewer2.org/#type=qualitative&scheme=Set1&n=6
        xlab("Distance from raw microbial comopostion") +
        ylab("Treatment group") +
        theme_classic (base_size = 12, base_family = "serif") + 
        theme(plot.tag = element_text(size = 15),
              axis.text.y = element_blank(),
              axis.ticks.y = element_blank(),
              legend.position = "top") +
        ggtitle("M-H distance from raw to decontaminated data, paired") +
        geom_vline(xintercept = 0, col = "black", linetype="dotted") +
        #coord_cartesian(xlim=c(-0.5, 1)) +
        #geom_text(aes(x = 0, label = treatment), hjust = 0, nudge_x = -.55, size = 3, color = "black", family = "serif") +
        #geom_text(aes(x = 0, label = text), hjust = 0, nudge_x = -0.4, size = 3, color = "black", family = "serif") +
...
png(file = "/Users/minsikkim/Dropbox (Partners HealthCare)/Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/FigureA5.png",   # The directory you want to save the file in
    width = 180, # The width of the plot in inches
    height = 180, # The height of the plot in inches
    units = "mm",
    res = 600
...
### PERMANOVA for decontaminated result

```{r}


horn_perm_simple <- vegan::adonis2(distance(phyloseq_rel_nz, method="horn") ~ subject_id + treatment + decontaminated,
                                  data = phyloseq_rel_nz %>% sample_data %>% data.frame(check.names = F),
                                  permutations = 10000)

horn_perm_all <- vegan::adonis2(distance(phyloseq_rel_nz, method="horn") ~ sample_type * treatment + subject_id + log10(Final_reads) + decontaminated,
                                  data = phyloseq_rel_nz %>% sample_data %>% data.frame(check.names = F),
                                  permutations = 10000)

horn_perm_ns <- vegan::adonis2(distance(subset_samples(phyloseq_rel_nz, sample_type == "Nasal"), method="horn") ~ lypma + benzonase + host_zero + molysis + qiaamp + log10(Final_reads) + decontaminated,
                               data = subset_samples(phyloseq_rel_nz, sample_type == "Nasal") %>%
                                       sample_data %>% data.frame(check.names = F),
                               strata = subset_samples(phyloseq_rel_nz, sample_type == "Nasal") %>% 
                                       sample_data %>% data.frame(check.names = F) %>% .$subject_id, permutations = 10000)

horn_perm_bal  <- vegan::adonis2(distance(subset_samples(phyloseq_rel_nz, sample_type == "BAL"), method="horn") ~  lypma + benzonase + host_zero + molysis + qiaamp + log10(Final_reads) + decontaminated,
                                 data = subset_samples(phyloseq_rel_nz, sample_type == "BAL") %>% sample_data %>% data.frame(check.names = F),
                                 strata = subset_samples(phyloseq_rel_nz, sample_type == "BAL") %>%
                                         sample_data %>% data.frame(check.names = F) %>% .$subject_id,
                                  permutations = 10000)

horn_perm_spt <- vegan::adonis2(distance(subset_samples(phyloseq_rel_nz, sample_type == "Sputum"), method="horn") ~ lypma + benzonase + host_zero + molysis + qiaamp + log10(Final_reads) + decontaminated,
                                data = subset_samples(phyloseq_rel_nz, sample_type == "Sputum") %>% sample_data %>% data.frame(check.names = F),
                                strata = subset_samples(phyloseq_rel_nz, sample_type == "Sputum")
                                %>% sample_data %>% data.frame(check.names = F) %>% .$subject_id,
                                  permutations = 10000)
...
tableA2 <- horn_perm_simple %>% data.frame(check.names = F) %>% rownames_to_column("row.names") %>% 
        mutate(row.names = case_when(row.names == "sample_type" ~ 'Sample type',
                                     row.names == "treatment" ~ 'Treatment',
                                     row.names == "subject_id" ~ 'Subject',
                                     row.names == "log10(Final_reads)" ~ 'log10(Final reads)',
                                     row.names == "decontaminated" ~ 'Decontamination',
                                     row.names == "sample_type:treatment" ~ 'Sample type * Treatment',
                                     row.names == "Residual" ~ 'Residual',
                                     row.names == "Total" ~ 'Total')) %>% column_to_rownames('row.names') %>% 
        mutate(` ` = case_when(abs(`Pr(>F)`) < 0.001 ~ "***",
