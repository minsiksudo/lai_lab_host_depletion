...
I am going to run MaAslin, but in this thread, MaAsLin showed different result with a CLR transformed data tested with GLM.
...
CLR transform was conducted with microbiome package
...
## Other transformation methods {.tabset}

### Amy's function for CLR normalization function
...
plot(x = maaslin_bal_transformed_otu$Cupriavidus_sp,
     y = physeq0_clr_bal %>% otu_table %>% t %>% data.frame() %>% .$Cupriavidus_sp, 
     xlab = "maaslin:: normalized CLR value",
     ylab = "microbiome:: normalized CLR value",
     main = "Cupriavidus CLR transformation")
...
# Running LMER for maaslin transfomred output
...
*q-value smaller than p-value!!* at LMER!

```{r}
maaslin_ns %>% subset(., .$feature == "Staphylococcus_aureus" & .$metadata == "lypma") 
```

q-val > p-val higher at MaAsLin

```{r}
lmer_maaslin_ns %>% 
        mutate(qval = qvalue(`Pr(>|t|)`)$qval) %>% 
        subset(., .$taxa == "Staphylococcus_aureus" & .$metadata == "lypma" & .$sample_type == "Nasal") 

```

q-val > p-val higher at LMER with MaAsLin transfored data.

# Volcano plot

```{r}

lmer_taxa_withzeros %>% subset(., .$sample_type == "BAL")  %>% 
        ggplot(., aes(x = Estimate, y = -log10(q_val))) +
        geom_point() +
        ggtitle("BAL LMER (with zeros) + CLR") +
        geom_hline(yintercept = 1, col = "red")

lmer_taxa_withzeros %>% subset(., .$sample_type == "Nasal")  %>% 
        ggplot(., aes(x = Estimate, y = -log10(q_val))) +
        geom_point() +
        ggtitle("Nasal LMER (with zeros) + CLR") +
        geom_hline(yintercept = 1, col = "red")
...
lmer_maaslin_bal_withzeros <- lmer_taxa_withzeros %>% 
        subset(., .$sample_type == "BAL") %>% 
                mutate(q_val = qvalue::qvalue(`Pr(>|t|)`)$qvalue)
        
lmer_maaslin_ns_withzeros <- lmer_taxa_withzeros %>% 
        subset(., .$sample_type == "Nasal") %>% 
                mutate(q_val = qvalue::qvalue(`Pr(>|t|)`)$qvalues)
        
        
lmer_maaslin_spt_withzeros <- lmer_taxa_withzeros %>%
        subset(., .$sample_type == "Sputum") %>%  
                mutate(q_val = qvalue::qvalue(`Pr(>|t|)`)$qvalue)
        

        
lmer_maaslin_ns_withzeros %>% subset(., .$taxa == "Staphylococcus_aureus" & .$metadata == "lypma" & .$sample_type == "Nasal") 
maaslin_ns %>% subset(., .$feature == "Staphylococcus_aureus" & .$metadata == "lypma") 
...
# Maybe, q-value is dependent on total number of tests, and it affected the result?
...
# How was it with zero-removed samples?
...
Now, at least, qval way higher than p-value.

```{r}


lmer_taxa_bal %>% 
        ggplot(., aes(x = Estimate, y = -log10(qval))) +
        geom_point() +
        ggtitle("BAL LMER (zero-filtered) + CLR, 25 significant taxa") +
        geom_hline(yintercept = 1, col = "red")

lmer_taxa_ns %>% 
        ggplot(., aes(x = Estimate, y = -log10(qval))) +
        geom_point() +
        ggtitle("Nasal LMER (zero-filtered) + CLR, 25 significant taxa") +
        geom_hline(yintercept = 1, col = "red")
...
lmer_taxa_spt %>% 
        ggplot(., aes(x = Estimate, y = -log10(qval))) +
        geom_point() +
        ggtitle("Sputum LMER (zero-filtered) + CLR, 25 significant taxa") +
        geom_hline(yintercept = 1, col = "red")
...
Now LMER and MaAslin got similar!
