...
**Controls chanced after treatment**
...
f_filt_data_bal <- read.csv("data/f_filt_data_bal.csv")
f_filt_data_spt <- read.csv("data/f_filt_data_spt.csv")
f_filt_data_ns <- read.csv("data/f_filt_data_ns.csv")
f_filt_data_pos <- read.csv("data/f_filt_data_pos.csv")
f_filt_data_neg <- read.csv("data/f_filt_data_neg.csv")

f_filt_data_bal_neg <- read.csv("data/f_filt_data_bal_neg.csv")
f_filt_data_spt_neg <- read.csv("data/f_filt_data_spt_neg.csv")
f_filt_data_ns_neg <- read.csv("data/f_filt_data_ns_neg.csv")
...
f_filt_data_bal <- make_sig_table(f_filt_data_bal)
...
spt_sig <- subset_taxa(subset_samples(phyloseq_rel_nz, sample_type == "Sputum"),
                               ...
                               gsub("-", ".", .) %in% f_fit_data_spt$data$feature)
f_fit_data_spt$rel <- cbind(spt_sig %>% otu_table %>% t, spt_sig %>% sample_data) %>% 
        group_by(treatment) %>%
...
ns_sig <- subset_taxa(subset_samples(phyloseq_rel_nz, sample_type == "Nasal"),
                               ...
                               gsub("-", ".", .) %in% f_fit_data_ns$data$feature)
...
f_fit_data_bal$rel <- cbind(bal_sig %>% otu_table %>% t, bal_sig %>% sample_data) %>% 
        group_by(treatment) %>%
        summarise_if(is.numeric, mean, na.rm = TRUE) %>% .[, 1:21] %>%
        column_to_rownames(., "treatment") %>% t () %>% data.frame(check.names = F) %>% 
        rownames_to_column("row") %>% mutate(row = gsub("-", ".", row)) %>% column_to_rownames("row") %>%
        .[row.names(f_fit_data_bal$data_italic),] %>%  mutate_all(~na_if(., 0)) %>% rownames_to_column("feature")
...
```{r warning=FALSE, message=FALSE}
ggballoonplot(f_fit_data_ns$rel %>% gather(treatment, value, Control:QIAamp, factor_key=TRUE), fill = "value", y = "feature", x= "treatment") +
        theme_classic(base_family = "serif") +
        scale_fill_viridis() +
        xlab("Experimental group") +
        ylab("Species") +
...
ggballoonplot(f_fit_data_spt$rel %>% gather(treatment, value, Control:QIAamp, factor_key=TRUE), fill = "value", y = "feature", x= "treatment") +
        theme_classic(base_family = "serif") +
        scale_fill_viridis() +
        xlab("Experimental group") +
        ylab("Species") +
...
ggballoonplot(f_fit_data_bal$rel %>% gather(treatment, value, Control:QIAamp, factor_key=TRUE), fill = "value", y = "feature", x= "treatment") +
        theme_classic(base_family = "serif") +
        scale_fill_viridis() +
        xlab("Experimental group") +
        ylab("Species") +
