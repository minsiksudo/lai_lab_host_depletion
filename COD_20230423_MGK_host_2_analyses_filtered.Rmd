...
Decontam ran with a new variable ((1-host ratio) * library concentration).
...
### List of DA taxa (Supplementary)

Table. List of differentailly abundant taxa identified by MaAsLin. Significant associations (q < 0.1) that is showing high change (|log2-fold change| > 0.5) were listed.

```{r, warning=FALSE, message=FALSE}

filt_maaslin_all %>% subset(., .$qval < 0.1 & abs(.$coef) > 0.5) %>% subset(., .$metadata != "sample_type" & .$metadata != "log10.Final_reads") %>% select(c("feature", "metadata", "coef", "qval")) %>%
        mutate(feature = paste("*",
                               gsub("_", " ", feature) %>% gsub(" sp", " sp.", .) %>% gsub(" group", "", .),
                               "*",
                               sep = "")
        ) %>% rename(Taxa = "feature",
                     Treatment = "metadata",
