...
ggarrange(sample_data %>% 
        ...
        data.frame() %>%
        mutate(`Host mapped / Raw reads` = 100 * sequencing_host_prop, 
        ...
        data.frame() %>%
        mutate(`Host mapped / Raw reads` = 100 * sequencing_host_prop, 
                `Host mapped / Total mapped` = Host_mapped / (Host_mapped + Final_reads) * 100) %>% 
...
***1. Library failure was associated with nasal swab, especially after
...
Shannon \~ sample_type \* treatment + log10 (Final_reads) +
(1\|original_sample)
...
BPI \~ sample_type + treatment + log10 (Final_reads) +
(1\|original_sample)
...
***p*** **- value = 0.059 for the interaction term.** **Interaction
term** will be tested.
...
**Nasal swab showed some sample type \* treatment specific effect**.
**Stratified analysis** will be conducted.
...
**Every covariate showed significant results against alpha diversity.
Evenness of sputum affected by treatments.**
...
***p*** **- value = 0.072 for the interaction term.** **Interaction
term** will be tested.
