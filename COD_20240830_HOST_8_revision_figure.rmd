...
table1_mock_and_neg <- sample_data %>% data.frame() %>% 
        ...
        summarise(`N` = n(),
            #      `Total DNA <br>ng/µL` = paste(format(round(median(picogreen_ng_ul),2), nsmall = 2, big.mark = ","), "<br>(", format(round(quantile(picogreen_ng_ul, 0.25),2), nsmall = 2, big.mark = ","), ", ", format(round(quantile(picogreen_ng_ul, 0.75),2), nsmall = 2, big.mark = ","), ")", sep = ""),
              ...
              `Template DNA <br>conc. (ng/µL)` = paste(format(round(median(picogreen_tem_ng_ul),1), nsmall = 1, big.mark = ","), " (", format(round(quantile(picogreen_tem_ng_ul, 0.25),1), nsmall = 1, big.mark = ","), ", ", format(round(quantile(picogreen_tem_ng_ul, 0.75),1), nsmall = 1, big.mark = ","), ")", sep = ""),
              `Library<br>conc. (ng/µL)` = paste(format(round(median(picogreen_lib_ng_ul),1), nsmall = 1, big.mark = ","), " (", format(round(quantile(picogreen_lib_ng_ul, 0.25),1), nsmall = 1, big.mark = ","), ", ", format(round(quantile(picogreen_lib_ng_ul, 0.75),1), nsmall = 1, big.mark = ","), ")", sep = ""),
              `Volume used for<br>pooling (µL)` = paste(format(round(median(pooling_vol_ul),1), nsmall = 1, big.mark = ","), " (", format(round(quantile(pooling_vol_ul, 0.25),1), nsmall = 1, big.mark = ","), ", ", format(round(quantile(pooling_vol_ul, 0.75),1), nsmall = 1, big.mark = ","), ")", sep = ""),
...
png(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/Figure6.png",   # The directory you want to save the file in
...
pdf(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/Figure6.pdf",   # The directory you want to save the file in
    width = 183 / 25.4, # Convert width from mm to inches (180 mm)
    height = 210 / 25.4, # Convert height from mm to inches (160 mm)
...
hdv_all %>%
     filter(
        ...
        #reactable::reactable(groupBy = c("species","treatment", "cfb_strain"))
...
### (Without glycerol) stratified by species (log scale)

LMER : --> removed DNA RNA shield from the model.

```{r}
hdv_all %>% 
        group_by(species, `DNA/RNA shield`, treatment2) %>%
        summarise (n = n())

# Analysis with glyceorl + unfrozen

multi_model_sa_glycerol <- hdv_all %>% 
                filter(
                species2 %in% c("Staphylococcus aureus"),
                `DNA/RNA shield` == 0,
                treatment2 %in% c("Unfrozen", "Frozen Glycerol", "Frozen Glycerol QIAamp", "Frozen Glycerol Benzonase",
                                  "Frozen glycerol HostZERO", "Frozen Glycerol lyPMA")
                ) %>%
        lmerTest::lmer(data = .,
                       cfu_per_ml_log10 ~ Frozen + treatment + (1|cfb_strain)) 
        

multi_model_pa_glycerol <- hdv_all %>% 
                filter(
                species2 %in% c("Pseudomonas aeruginosa"),
                `DNA/RNA shield` == 0,
                treatment2 %in% c("Unfrozen", "Frozen Glycerol", "Frozen Glycerol QIAamp", "Frozen Glycerol Benzonase",
...
multi_model_a_glycerol <- hdv_all %>% 
                filter(
                species2 %in% c("Achromobacter spp"),
                `DNA/RNA shield` == 0,
                treatment2 %in% c("Unfrozen", "Frozen Glycerol", "Frozen Glycerol QIAamp", "Frozen Glycerol Benzonase",
...
multi_model_sa_nogly <- hdv_all %>% 
                filter(
                species2 %in% c("Staphylococcus aureus"),
                `DNA/RNA shield` == 0,
                treatment2 %in% c("Unfrozen", "Frozen", "Frozen QIAamp", "Frozen Benzonase",
                                  "Frozen HostZERO", "Frozen lyPMA")
                ) %>%
        lmerTest::lmer(data = .,
                       cfu_per_ml_log10 ~ Frozen + treatment + (1|cfb_strain)) 
        

multi_model_pa_nogly <- hdv_all %>% 
                filter(
                species2 %in% c("Pseudomonas aeruginosa"),
                `DNA/RNA shield` == 0,
                treatment2 %in% c("Unfrozen", "Frozen", "Frozen QIAamp", "Frozen Benzonase",
...
multi_sa_gly <- multi_model_sa_glycerol %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
        ...
        rename("<i>p</i>-value" = "Pr(>|t|)",
               t_value = "t value") %>%
        merge(., 
              multi_model_sa_glycerol %>% confint() %>%
                        round(digits = 1) %>% 
                        format(nsmall = 1) %>%
                        as.data.frame() %>%
                        mutate(conf = paste("(",
...
multi_pa_glycerol <- multi_model_pa_glycerol %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
        ...
        rename("<i>p</i>-value" = "Pr(>|t|)",
               t_value = "t value") %>%
        merge(., 
              multi_model_pa_glycerol %>% confint() %>%
                        round(digits = 1) %>% 
                        format(nsmall = 1) %>%
                        as.data.frame() %>%
                        mutate(conf = paste("(",
...
multi_a_glycerol <- multi_model_a_glycerol %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
        ...
        rename("<i>p</i>-value" = "Pr(>|t|)",
               t_value = "t value") %>%
        merge(., 
              multi_model_a_glycerol %>% confint() %>%
                        round(digits = 1) %>% 
                        format(nsmall = 1) %>%
                        as.data.frame() %>%
                        mutate(conf = paste("(",
...
multi_sa_nogly <- multi_model_sa_nogly %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
...
multi_pa_nogly <- multi_model_pa_nogly %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
...
multi_a_nogly <- multi_model_a_nogly %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.001 ~ "***",
...
cbind(multi_sa_gly, multi_pa_glycerol, multi_a_glycerol) 

cbind(multi_sa_nogly, multi_pa_nogly, multi_a_nogly) 


data <- cbind(multi_sa_gly, multi_pa_glycerol, multi_a_glycerol,multi_sa_nogly, multi_pa_nogly, multi_a_nogly) 
#rownames(data) <- c("(Intercept)","Frozen", "lyPMA", "Benzonase","HostZERO","MolYsis","QIAamp" )

lmer_viability_result <- data %>%
    kbl(format = "html", escape = 0) %>%
        add_header_above(c(" " = 1, "<i>Staphylococcus aureus</i>" = 3, "<i>Pseudomonas aeruginosa</i>" = 3,"<i>Achromobacter</i> spp."= 3,
                           "<i>Staphylococcus aureus</i>" = 3, "<i>Pseudomonas aeruginosa</i>" = 3,"<i>Achromobacter</i> spp."= 3),
                         escape = F) %>%
        add_header_above(c(" " = 1, "Cryoprotected"= 9,"Not cryoprotected"= 9),escape = F) %>%
        kable_styling(full_width = 0, html_font = "sans")

lmer_viability_result


save_kable(lmer_viability_result, file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/tableS12-v3.html", self_contained = T)
...
read_excel("Summer_Student_Projects/People/@thi/SOP/TAT_Host_Depletion/SOP_20240924_TAT_HDV_DNA_extraction.xlsx")
qPCR1 <- read_excel("Summer_Student_Projects/People/@thi/SOP/TAT_Host_Depletion/SOP_20240927_TAT_HDV_16S_qPCR.xlsx", sheet = 1)
qPCR2 <- read_excel("Summer_Student_Projects/People/@thi/SOP/TAT_Host_Depletion/SOP_20240927_TAT_HDV_16S_qPCR.xlsx", sheet = 10, skip= 24)
qPCR1

qPCR2 %>%
        group_by(Sample) %>%
        summarise(mean = mean(as.numeric(Cq), na.rm = T),
                  sd = sd(Cq),
                  max = max(Cq),
                  median = median(Cq),
                  min = min(Cq))
...
qPCR <- merge(
        ...
        qPCR2 %>%
        group_by(Sample) %>%
        summarise(cq_mean = mean(as.numeric(Cq)),
                  sd_mean = sd(as.numeric(Cq))) %>%
                rename(expt_id = Sample), 
        by = "expt_id",
        all = T)


standard <- subset(qPCR, grepl("STD", qPCR$expt_id)) %>%
         subset(., !grepl("STD8", .$expt_id))
        
standard$dna_ng_well <- c(20,2, 0.2, 0.02, 0.002, 0.0002, 0.00002)

standard$dna_ng_ul_temp <- c(10,1, 0.1, 0.01, 0.001, 0.0001, 0.00001)
        
model <- lm(log2(standard$dna_ng_ul_temp) ~ standard$cq_mean) %>% summary




standard$dna_ng_ul_tem_log_dil <- (model$coefficients[1] + standard$cq_mean * (model$coefficients[2]))
standard$dna_ng_ul_tem_dil <- 2^(standard$dna_ng_ul_tem_log)
standard$dna_ng_ul_tem_nondil <- standard$dna_ng_ul_tem_dil *10




qPCR$dna_ng_ul_tem_log_dil <- (model$coefficients[1] + qPCR$cq_mean * (model$coefficients[2]))
qPCR$dna_ng_ul_tem_dil <- 2^(qPCR$dna_ng_ul_tem_log)
qPCR$dna_ng_ul_tem_nondil <- qPCR$dna_ng_ul_tem_dil *10

standard  %>%
        subset(., grepl("STD", .$expt_id)) %>%
         subset(., !grepl("STD8", .$expt_id)) %>%
        ggplot(., aes(x = log10(dna_ng_well), y = log10(dna_ng_ul_tem_dil * 2))) +
        geom_point() +
        theme_classic() +
        xlab("Defined concentration, per well (ng/μL)") +
        ylab("Concentration from linear mdoel, per well (ng/μL)")



qPCR  %>%
        ggplot(., aes(x = sample_type, y = dna_ng_ul_tem_nondil)) +
        geom_jitter(aes(col = sample_type, x = sample_type),
                    lwd = 0.2, alpha = 0.3, stroke = 0) +
        stat_summary(aes(color = sample_type, x = sample_type),
                             fun.data="mean_sdl",  fun.args = list(mult=1), 
                             geom = "pointrange",  size = 0.4) +
        theme_classic() +
        scale_color_manual(values = c("#e31a1c", "#1f78b4", "#33a02c", "#b2df8a")) +
        facet_wrap(~species, scales = "free_y", ncol = 4) + #color using https://colorbrewer2.org/#type=qualitative&scheme=Set1&n=6
        #scale_x_discrete(name ="Treatment")+
        guides(fill = guide_legend(nrow = 1),
               color = guide_legend(nrow = 1)) +
        ...
        #plot.tag = element_text(size = 15)) +
              #axis.text.x = element_blank()) +
        ylab("qPCR bacterial DNA (ng/μL)") +
        xlab("Sample type")

#All = all the culture --> DNA extraction
#Frozen --> Frozen pellet - is it decreasing? 

#Pellet --> raw pellet

#supernatnat  --> filtered sup and DNA extracted. --> extracellular DNA
#Pellet --> intracellular DNA



qPCR_wide <- qPCR %>%
        group_by(cfb_strain, sample_type) %>%
        summarise(species = species,
                  dna_ng_ul_tem_nondil = dna_ng_ul_tem_nondil) %>% 
        filter(!is.na(cfb_strain)) %>% 
        spread(., sample_type, dna_ng_ul_tem_nondil)
        

#From zymo... (https://zymoresearch.eu/products/femto-bacterial-dna-quantification-kit?srsltid=AfmBOoo_qYIhDhpNHjgEgsTLMIxZ4Bg_9mMibbVWJgdqNU6RZZqCEIM5)
...
#Genome copy number = (Amount of DNA (g)) / (g to bp constant * Genome size (bp))
#Genome copy # = DNA (g) / (g_to_bp const. x genome size) With DNA (g) = 20 ng g to bp const = 1.096 x 10^-21 ng genome size = 4.6 x 10^6 bp 20 x 10^-9 g / (1.096 x 10^-21 g/bp x 4.6 x 10^6 bp) = 4.0 x 10^6 copies

genome_size_bp_ecoli <- 4.6e6  # Genome size in base pairs (bp)
genome_size_bp_sa <- 2839469  # https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4654806/
genome_size_bp_pa <- c(6.264, 6.538, 6.588, 6.602, 6.492, 6.905, 6.222, 6.667) * 1000000 #https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3139241/#:~:text=The%20Pseudomonas%20aeruginosa%20genome%20(G,a%20variable%20number%20of%20plasmids.
genome_size_bp_a <- 5876039 #https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4543513/

bp_to_g_constant <- 1.096e-21  # g/bp constant

g_to_bp_constant <- 9.26e20

e_coli_16S_copies <- 7 

# Number of 16S rRNA gene copies per each species
copies_per_genome <- data.frame(speices = c("Staphylococcus aureus",
                                           "Pseudomonas aeruginosa",
                                           "Achromobacter sp"),
                               genome_16S_copies = c(5.7, 4, 3.5)) %>%
        column_to_rownames("speices")


# Number of 16S rRNA gene copies per each species
genome_size <- data.frame(speices = c("Staphylococcus aureus",
                                           "Pseudomonas aeruginosa",
                                           "Achromobacter sp"),
                               genome_size = c(genome_size_bp_sa, mean(genome_size_bp_pa), genome_size_bp_a)) %>%
        column_to_rownames("speices")



qPCR_wide$copies_per_genome <- copies_per_genome[qPCR_wide$species,]

qPCR_wide$genome_size <- genome_size[qPCR_wide$species,]

qPCR_wide %>% 
        mutate(prop = SUPERNATANT/(SUPERNATANT+PELLET)*100) %>%
        view
...
# this is 16S copy per / uL
qPCR_wide_dna_amount_g <- qPCR_wide %>%
        mutate(ALL = ALL * 1e-9,
               FROZEN = FROZEN * 1e-9,
               PELLET = PELLET * 1e-9,
               SUPERNATANT = SUPERNATANT * 1e-9)

qPCR_wide_basepairs <- qPCR_wide_dna_amount_g %>%
        mutate(ALL = ALL * g_to_bp_constant,
               FROZEN = FROZEN * g_to_bp_constant,
               PELLET = PELLET * g_to_bp_constant,
               SUPERNATANT = SUPERNATANT * g_to_bp_constant)

qPCR_wide_ecoli_genome <- qPCR_wide_basepairs %>%
        mutate(ALL = (ALL)/genome_size_bp_ecoli,
               FROZEN = (FROZEN)/genome_size_bp_ecoli,
               PELLET = (PELLET)/genome_size_bp_ecoli,
               SUPERNATANT = (SUPERNATANT)/genome_size_bp_ecoli)


qPCR_wide_16S_copies <- qPCR_wide_ecoli_genome %>%
        mutate(ALL = (7 * ALL),
               FROZEN = (7 * FROZEN),
               PELLET = (7 * PELLET),
               SUPERNATANT = (7 * SUPERNATANT))

qPCR_wide_genome_copies <- qPCR_wide_16S_copies %>%
        mutate(ALL = ALL/copies_per_genome,
               FROZEN = FROZEN/copies_per_genome,
               PELLET = PELLET/copies_per_genome,
               SUPERNATANT = SUPERNATANT/copies_per_genome)

qPCR_cells_figure <- qPCR_wide_genome_copies %>% 
        gather(., sample_type, cells, ALL:SUPERNATANT) %>%
        filter(sample_type %in% c("PELLET", "SUPERNATANT")) %>%
        mutate(sample_type = case_when(sample_type == "PELLET" ~ "Pellet",
                                       sample_type == "SUPERNATANT" ~ "Supernatant"),
               species = factor(species, levels = c("Staphylococcus aureus",
                                                    "Pseudomonas aeruginosa",
                                                    "Achromobacter sp"
                                                    ))) %>%
        ggplot(., aes(x = sample_type, y = cells)) +
        geom_jitter(aes(col = sample_type, x = sample_type),
                    lwd = 0.2, alpha = 0.3, stroke = 0) +
        stat_summary(aes(color = sample_type, x = sample_type),
                             fun.data="mean_sdl",  fun.args = list(mult=1), 
        ...
        facet_wrap(~species, scales = "free_y", ncol = 4,
                   labeller = as_labeller(c(
                        "Staphylococcus aureus" = "*Staphylococcus aureus*",
                        "Pseudomonas aeruginosa" = "*Pseudomonas aeruginosa*",
                        "Achromobacter sp" = "*Achromobacter* spp."))) + #color using https://colorbrewer2.org/#type=qualitative&scheme=Set1&n=6
        #scale_x_discrete(name ="Treatment")+
        guides(fill = guide_legend(nrow = 1),
               color = guide_legend(nrow = 1, title = "Smaple type")) +
        theme(legend.position = "top",
              axis.title.y = element_markdown(),
        ...
        #plot.tag = element_text(size = 15)) +
              #axis.text.x = element_blank()) +
        ylab("Genome copies by qPCR (copies/μL)") +
        xlab("Sample type") 
...
#Calculate percentage of extrazcellular DNA 
qPCR_prop_figure <- qPCR_wide_genome_copies %>%
        mutate(eDNA_prop = SUPERNATANT/(SUPERNATANT+PELLET) * 100) %>%
        mutate(species = factor(species, levels = c("Staphylococcus aureus",
                                                    "Pseudomonas aeruginosa",
                                                    "Achromobacter sp"
                                                    ),
        ...
        ggplot(., aes(x = species, y = eDNA_prop)) +
        geom_jitter(aes(col = species, x = species),
                    lwd = 0.2, alpha = 0.3, stroke = 0) +
        stat_summary(aes(color = species, x = species),
                             fun.data="mean_sdl",  fun.args = list(mult=1), 
                             geom = "pointrange",  size = 0.4) +
        theme_classic (base_size = 12, base_family = "sans")+ 
        guides(fill = guide_legend(nrow = 1),
               color = guide_legend(nrow = 1, title = "Species", position = "top")) +
        theme(axis.text.x = element_markdown(), legend.text = element_markdown(),
              legned.position = "top") +
        ylab("Proportion of extracellular DNA (%)") +
        xlab("Species") +
...
png(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/FigureS18.png",   # The directory you want to save the file in
    width = 190, # The width of the plot in inches
...
ggarrange(qPCR_cells_figure, qPCR_prop_figure, ncol = 1)
...
qPCR_cells_table <- qPCR_wide_genome_copies %>% 
        mutate(prop = SUPERNATANT/(SUPERNATANT+PELLET) * 100,
                species = factor(species, levels = c("Staphylococcus aureus",
                                                    "Pseudomonas aeruginosa",
                                                    "Achromobacter sp"
                                                    ),
                                labels = c("<i>Staphylococcus aureus</i>",
                                                    "<i>Pseudomonas aeruginosa</i>",
                                                    "<i>Achromobacter</i> spp."
                                                    ))) %>%
        group_by(species) %>%
        summarise(`Genome in pellet (copies / µL)` = 
                          paste0(
                                  mean(PELLET) %>% round(1) %>% format(., nsmall = 1, 
                                                                       big.mark=",", 
                                                                       scientific=F),
                                  " ± ",
                                  sd(PELLET) %>% round(1)) %>% format(., nsmall = 1, 
                                                                       big.mark=",", 
                                                                       scientific=F),
                  `Genome in supernatant (copies / µL)` = 
                          paste0(
                                  mean(SUPERNATANT) %>% round(1) %>% format(., nsmall = 1, 
                                                                       big.mark=",", 
                                                                       scientific=F),
                                  " ± ",
                                  sd(SUPERNATANT) %>% round(1)) %>% format(., nsmall = 1, 
                                                                       big.mark=",", 
                                                                       scientific=F),
                  `Proportion of extracellular DNA (%)` = 
                          paste0(
                                  mean(prop) %>% round(1) %>% format(., nsmall = 1, 
                                                                       big.mark=",", 
                                                                       scientific=F),
                                  " ± ",
                                  sd(prop) %>% round(1) %>% format(., nsmall = 1, 
                                                                       big.mark=",", 
                                                                       scientific=F))) %>%
        kbl(format = "html", escape = 0) %>%
        #add_header_above(c(" " = 1, "BAL" = 3, "Nasal swab" = 3,"Sputum"= 3)) %>% 
...
qPCR_cells_table %>%
        save_kable(., 
                   file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/tableC21.html", self_contained = T)



qPCR_wide_genome_copies %>% 
        mutate(prop = SUPERNATANT/(SUPERNATANT+PELLET) * 100,
               species = factor(species, 
                                 levels = c("Staphylococcus aureus",
                                            "Pseudomonas aeruginosa",
                                            "Achromobacter sp"))) %>%
         mutate(gram_negative = case_when(species == "Staphylococcus aureus" ~ 0,
                                          .default = 1) ) %>%
         data.frame() %>%
         lm(prop ~ species, data = .) %>%
         summary
