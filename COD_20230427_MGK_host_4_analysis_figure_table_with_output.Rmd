...
png(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/FigureS2.png",   # The directory you want to save the file in
    width = 180, # The width of the plot in inches
        ...
        summarise(`N` = n(),
            #      `Total DNA <br>ng/µL` = paste(format(round(median(picogreen_ng_ul),2), nsmall = 2, big.mark = ","), "<br>(", format(round(quantile(picogreen_ng_ul, 0.25),2), nsmall = 2, big.mark = ","), ", ", format(round(quantile(picogreen_ng_ul, 0.75),2), nsmall = 2, big.mark = ","), ")", sep = ""),
             ...
             `Final reads<br>reads x 10<sup>6</sup>` = paste(format(round(median(Final_reads/1000000,
                                                             ...
                                                             ")",
                                                             sep = ""),
             `Microbial<br>species<br>richness` = paste(median(S.obs, na.rm = T),
                                                             " (", 
                                                             ...
                                                             ")",
                                                             sep = ""),
            `Microbial function<br>richness` = paste(median(F.obs, na.rm = T),
                                                             " (", 
                                                             ...
                                                             ")",
                                                             sep = ""),
                `Viral<br>species<br>richness` = paste(median(V.obs, na.rm = T),
                                                             " (", 
                                                             quantile(V.obs, 0.25, na.rm = T),
                                                             ", ",
                                                             quantile(V.obs, 0.75, na.rm = T),
                                                             ")",
                                                             sep = "")
...
png(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/FigureS6.png",   # The directory you want to save the file in
    width = 180, # The width of the plot in inches
...
sr_lmer_bal_v <- lmer(V.obs ~ treatment + (1|subject_id),
                    data = sample_data %>% 
                            data.frame %>% 
                            subset(., .$sample_type %in% c("BAL") & S.obs != 0))
...
sr_lmer_ns_v <- lmer(V.obs ~ treatment + (1|subject_id),
                   data = sample_data %>% 
                           data.frame %>% 
                           subset(., .$sample_type %in% c("Nasal")))
...
```{r}
sr_lmer_spt_v <- lmer(V.obs ~ treatment + (1|subject_id),
                    data = sample_data %>% 
                            data.frame %>% 
                            subset(., .$sample_type %in% c("Sputum")))
...
set.seed(seed)
horn_perm_inter_fun <- vegan::adonis2(distance(phyloseq_rel_nz_f, method="horn") ~ sample_type * treatment + subject_id,
                                  ...
                                  permutations = 10000)
set.seed(seed)
horn_perm_ns_fun <- vegan::adonis2(distance(subset_samples(phyloseq_rel_nz_f, sample_type == "Nasal"), method="horn") ~ lypma + benzonase + host_zero + molysis + qiaamp,
                               data = subset_samples(phyloseq_rel_nz_f, sample_type == "Nasal") %>%
                                       sample_data %>% data.frame(check.names = F),
                               strata = subset_samples(phyloseq_rel_nz_f, sample_type == "Nasal") %>% 
                                       sample_data %>% data.frame(check.names = F) %>% .$subject_id, permutations = 10000)
set.seed(seed)
horn_perm_bal_fun  <- vegan::adonis2(distance(subset_samples(phyloseq_rel_nz_f, sample_type == "BAL"), method="horn") ~  lypma + benzonase + host_zero + molysis + qiaamp,
                                 ...
                                 strata = subset_samples(phyloseq_rel_nz_f, sample_type == "BAL") %>%
                                  ...
                                  permutations = 10000)
set.seed(seed)
horn_perm_spt_fun <- vegan::adonis2(distance(subset_samples(phyloseq_rel_nz_f, sample_type == "Sputum"), method="horn") ~ lypma + benzonase + host_zero + molysis + qiaamp,
...
png(file = "Project_SICAS2_microbiome/7_Manuscripts/2022_MGK_Host_Depletion/Figures/FigureS13.png",   # The directory you want to save the file in
    width = 180, # The width of the plot in inches
    height = 180, # The height of the plot in inches
    units = "mm",
    res = 600
...
## Viral outcome {.tabset}


### Viral species richness (LMER)

```{r}


vr_lmer_bal <- lmer(V.obs ~ treatment + (1|subject_id),
                    data = sample_data %>% 
                            data.frame %>% 
                            subset(., .$sample_type %in% c("BAL") & V.obs != 0))
        


vr_lmer_ns <- lmer(V.obs ~ treatment + (1|subject_id),
                    data = sample_data %>% 
                            data.frame %>% 
                            subset(., .$sample_type %in% c("Nasal") & V.obs != 0))



vr_lmer_spt <- lmer(S.obs ~ treatment + (1|subject_id),
                    data = sample_data %>% 
                            data.frame %>% 
                            subset(., .$sample_type %in% c("Sputum") & S.obs != 0))
