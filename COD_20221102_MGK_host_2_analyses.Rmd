...
***Analysis***
	...
	--> both stratified and nonstratified
        ...
        5. DA analysis for function, by sample type and treatment 
	--> both stratified and nonstratified
	--> multiple levels?
...
#Relative abundacne transfromation
...
*2.1.1 Modeling final read should be conducted with both linear and logistic regression*
...
taxa_qc <- data.frame("species" =  otu_table(phyloseq_rel) %>% t() %>% colnames(),
        "prevalence" = ifelse(phyloseq %>% otu_table() > 0, 1, 0) %>% t() %>% colSums(), #Prevalence of taxa
        "mean_rel_abd" = phyloseq_rel %>% otu_table() %>% t() %>% colMeans(na.rm = T) #mean relativ abundacne 
)
...
red_flag_taxa <- data.frame(species = taxa_qc$species,
                          red_flag_prev_abd = ifelse(taxa_qc$prevalence < otu_table(phyloseq_rel) %>% t %>% rownames() %>% length * 0.05 & taxa_qc$mean_rel_abd < quantile(taxa_qc$mean_rel_abd, 0.75), 1, 0)
)
```
