...
```{r, echo=FALSE,message=FALSE, results='hide',fig.keep='all', "MaAslin taxa"}
...
#all samples
capture.output(maaslin_all = Maaslin2(input_data = otu_table(phyloseq_rel_nz) %>% t %>% data.frame(), 
...
capture.output(fit_data_ns = Maaslin2(input_data = otu_table(subset_samples(phyloseq_rel_nz, sample_type == "Nasal swab")) %>% t %>% data.frame(),
...
capture.output(fit_data_bal = Maaslin2(input_data = otu_table(subset_samples(phyloseq_rel_nz, sample_type == "BAL")) %>% t %>% data.frame(),
...
capture.output(fit_data_spt = Maaslin2(input_data = otu_table(subset_samples(phyloseq_rel_nz, sample_type == "Sputum")) %>% t %>% data.frame(),
...
* MaAsLin interaction term *


```{r, echo=FALSE,message=FALSE, results='hide',fig.keep='all', "MaAslin taxa interaction"}
...
```{r, echo=FALSE,message=FALSE, results='hide',fig.keep='all', "MaAslin genus & family level"}
...
```{r warning=FALSE, "Function alpha diversity"}
...
bray_perm_strata %>% data.frame(check.names = F) %>% rownames_to_column('row.names') %>% 
        ...
        kbl(format = "html") %>%
        kable_styling(full_width = 0, html_font = "serif")
cat("\n Strata results: showing higher p values. None of the treatment methods significantly affected the beta-diversity index\n")
...
```{r, echo=FALSE,message=FALSE, results='hide',fig.keep='all', "MaAslin function"}
sample_data(phyloseq_rel_nz)$log10.Final_reads <- log10(sample_data(phyloseq_rel_nz)$Final_reads)
sample_data(phyloseq_rel_nz)$sampletype_treatment <- paste(sample_data(phyloseq_rel_nz)$sample_type, sample_data(phyloseq_rel_nz)$treatment, sep = ":")
