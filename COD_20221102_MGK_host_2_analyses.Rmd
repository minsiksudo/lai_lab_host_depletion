...
*2.1. 13 samples failed in library prep*

*2.1.2. Two BAL sampels showed 0 total reads*

*2.1.3. Sequencing fail ≠ library prep fail*
...
#               feature = factor(feature, levels = c("Raw_reads", "LowQual_removed", "Reads_after_trim", "Host_mapped", "Final_reads", "Metaphlan_mapped", #"sequencing_host_prop"), labels = c("Raw reads", "Low qual removed", "Reads after trim", "Host mapped", "Final reads", "Metaphlan mapped", "Host %")),
...
#                scale_fill_manual(values = c("#e41a1c", "#377eb8", "#4daf4a", "#984ea3", "#ff7f00", "#ffff33"), name = "Treatment") #color using
...
phyloseq_control_rel <- rbind(c("mock_theoretical", "mock")) %>% data.frame() %>%
        ...
        merge_phyloseq(., subset_samples(phyloseq, sample_type == "Pos. ext." | sample_type == "Neg. ext." )) %>% #adding data of controls
...
2.4.1. run decontam for all samples
...
#running decontam - prevalence method
...
sample_data(phy_decontam)$DNA_bac_nondil <- ifelse(sample_data(phy_decontam)$DNA_bac_nondil %>% is.na(), 0.01, sample_data(phy_decontam)$DNA_bac_nondil)
sample_data(phy_decontam)$total_dna_ngul <- ifelse(sample_data(phy_decontam)$total_dna_ngul %>% is.na(), 0.01, sample_data(phy_decontam)$total_dna_ngul)
