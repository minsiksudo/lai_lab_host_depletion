...
#Checking number of bugs for the main text of the manuscript 
cat("Number of differentially abundant bugs by each metadata")
maaslin_all2$results %>% subset(., .$qval < 0.1) %>% .$metadata %>% table()
cat("Decreased bugs by each metadata")
maaslin_all2$results %>% subset(., .$qval < 0.1 & .$coef < 0) %>% .$metadata %>% table()
cat("Incrased")
maaslin_all2$results %>% subset(., .$qval < 0.1 & .$coef > 0) %>% .$metadata %>% table()
...
#Checking number of bugs for the main text of the manuscript 
cat("Number of differentially abundant bugs by each metadata in NS")
fit_data_ns2$results %>% subset(., .$qval < 0.1) %>% .$metadata %>% table()
...
cat("Number of differentially abundant bugs by each metadata in BAL")
fit_data_bal2$results %>% subset(., .$qval < 0.1) %>% .$metadata %>% table()
...
cat("Number of differentially abundant bugs by each metadata in sputum")
fit_data_spt2$results %>% subset(., .$qval < 0.1) %>% .$metadata %>% table()
...
ggplot(maaslin_all2$results, aes(y = -log10(qval), x = coef, col = metadata)) +
        theme_classic(base_family = "serif") +
        #labs(tag = "A") +
        ggtitle("MaAslin with treatment types")+
        geom_point(size = 2) +
...
fit_data_bal <- make_sig_table(fit_data_bal2)
fit_data_ns <- make_sig_table(fit_data_ns2)
fit_data_spt <- make_sig_table(fit_data_spt2)
