...
```{r}
lmer(data_invsimpson ~ sample_type * treatment + (1|original_sample), data = subset(sample_data, sample_data$sample_type == "Sputum" |sample_data$sample_type == "Mock" | sample_data$sample_type == "Neg.")) %>% 
        summary() %>%
        .$coefficients %>%
        data.frame(check.names = F) %>% 
        mutate(` ` = case_when(abs(`Pr(>|t|)`) < 0.05 ~ "*",
...
### Stratified with controls


```{r}


bray_perm_bal_ctrl %>% data.frame(check.names = F) %>% rownames_to_column('row.names') %>% 
        mutate(row.names = case_when(row.names == "sample_type" ~ 'Sample type',
                                     row.names == "lypma" ~ 'lyPMA',
                                     row.names == "benzonase" ~ 'Benzonase',
                                     row.names == "host_zero" ~ 'Host zero',
        ...
        round(3) %>% mutate(` ` = case_when(abs(`Pr(>F)`) < 0.05 ~ "*",
                               .default = " ")) %>% 
        kbl(format = "html",label = "BAL with controls") %>%
        kable_styling(full_width = 0, html_font = "serif")

bray_perm_ns_ctrl %>% data.frame(check.names = F) %>% rownames_to_column('row.names') %>% 
        mutate(row.names = case_when(row.names == "sample_type" ~ 'Sample type',
                                     row.names == "lypma" ~ 'lyPMA',
                                     row.names == "benzonase" ~ 'Benzonase',
                                     row.names == "host_zero" ~ 'Host zero',
        ...
        round(3) %>% mutate(` ` = case_when(abs(`Pr(>F)`) < 0.05 ~ "*",
                               .default = " ")) %>% 
        kbl(format = "html",label = "Nasal with controls") %>%
        kable_styling(full_width = 0, html_font = "serif")


bray_perm_spt_ctrl %>% data.frame(check.names = F) %>% rownames_to_column('row.names') %>% 
        mutate(row.names = case_when(row.names == "sample_type" ~ 'Sample type',
                                     row.names == "lypma" ~ 'lyPMA',
                                     row.names == "benzonase" ~ 'Benzonase',
                                     row.names == "host_zero" ~ 'Host zero',
        ...
        round(3) %>% mutate(` ` = case_when(abs(`Pr(>F)`) < 0.05 ~ "*",
                               .default = " ")) %>% 
        kbl(format = "html",label = "Sputum with controls") %>%
        kable_styling(full_width = 0, html_font = "serif")
...
subset_samples(phyloseq_rel_nz, treatment == "lyPMA" & sample_type != "Mock") %>%
        isContaminant(., method="prevalence", neg = "is.neg", threshold = 0.5) %>% subset(.,.$contaminant)
cat("decontam freq - lyPMA")
subset_samples(phyloseq_rel_nz, treatment == "lyPMA" & sample_type != "Mock") %>%
        isContaminant(method="frequency", conc="DNA_bac_well") %>% subset(.,.$contaminant)
cat("decontam comb - lyPMA")
subset_samples(phyloseq_rel_nz, treatment == "lyPMA" & sample_type != "Mock") %>%
        isContaminant(method="combined", neg="is.neg", conc = "DNA_bac_well") %>% subset(.,.$contaminant)



cat("decontam prev - Benzonase")
subset_samples(phyloseq_rel_nz, treatment == "Benzonase" & sample_type != "Mock") %>%
        isContaminant(., method="prevalence", neg = "is.neg", threshold = 0.5) %>% subset(.,.$contaminant)

cat("decontam freq - Benzonase")
subset_samples(phyloseq_rel_nz, treatment == "Benzonase" & sample_type != "Mock") %>%
        isContaminant(method="frequency", conc="DNA_bac_well") %>% subset(.,.$contaminant)

cat("decontam comb - Benzonase")
subset_samples(phyloseq_rel_nz, treatment == "Benzonase" & sample_type != "Mock") %>%
        isContaminant(method="combined", neg="is.neg", conc = "DNA_bac_well") %>% subset(.,.$contaminant)



cat("decontam prev - Molysis")
subset_samples(phyloseq_rel_nz, treatment == "Molysis" & sample_type != "Mock") %>%
        isContaminant(., method="prevalence", neg = "is.neg", threshold = 0.5) %>% subset(.,.$contaminant)

cat("decontam ferq - Molysis")
subset_samples(phyloseq_rel_nz, treatment == "Molysis" & sample_type != "Mock") %>%
        isContaminant(method="frequency", conc="DNA_bac_well") %>% subset(.,.$contaminant) 
...
cat("decontam prev - Host zero")
subset_samples(phyloseq_rel_nz, treatment == "Host zero" & sample_type != "Mock") %>%
        isContaminant(., method="prevalence", neg = "is.neg", threshold = 0.5) %>% subset(.,.$contaminant)

cat("decontam freq - Host zero")
subset_samples(phyloseq_rel_nz, treatment == "Host zero" & sample_type != "Mock") %>%
        isContaminant(method="frequency", conc="DNA_bac_well") %>% subset(.,.$contaminant) 
...
cat("decontam prev - QIAamp")
subset_samples(phyloseq_rel_nz, treatment == "QIAamp" & sample_type != "Mock") %>%
        isContaminant(., method="prevalence", neg = "is.neg", threshold = 0.5) %>% subset(.,.$contaminant)

cat("decontam freq - QIAamp")
subset_samples(phyloseq_rel_nz, treatment == "QIAamp" & sample_type != "Mock") %>%
        isContaminant(method="frequency", conc="DNA_bac_well") %>% subset(.,.$contaminant) 
